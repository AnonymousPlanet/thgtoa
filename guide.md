...
Version 0.6.8 (draft), February 2021 (work in progress, some parts are incomplete) by AnonymousPlanet.

**Remember to check the frequently for new versions of this guide.**
...
(See this good article on the matter <https://www.privateinternetaccess.com/blog/how-does-privacy-differ-from-anonymity-and-why-are-both-important/>)
...
You might think this guide has no legitimate use but there are many[^11]'[^12]'[^13]'[^14] such as:
...
**Use at your own risk. Anything in here is not legal advice and you should verify compliance with your local law before use (IANAL**[^15]**). "Trust but verify"**[^16] **all the information in here.**

# Requirements:

-   **Be a permanent Adult resident in Germany where the courts have upheld up the legality of not using real names on online platforms (§13 VI of the German Telemedia Act of 2007**[^17]**). Alternatively be an Adult resident of any other country where you can validate and verify the legality of this guide yourself.**
...
-   Don't be evil (for real this time)[^18].
...
-   Using the Tor Anonymity Network[^21] (free).

-   Using a VPN[^22] service anonymously (anonymously paid).

All those will be explained later in this guide.

## Your DNS requests:

DNS stands for "Domain Name System"[^23] and is a service used by your browser (and other apps) to find the IP addresses of a service. It's pretty much a huge "contact list" (phone book for older people) that works like asking it a name and it returns the number to call. Except it returns an IP instead.
...
Usually the DNS service is provided by your ISP and automatically configured by the network you're connecting to. This DNS service could also be subject to data retention regulations or will just keep logs for other reasons (data collection for advertising purposes for instance). Therefore this ISP will be capable of telling everything you did online just by looking at those logs which can in turn be provided to an adversary. Conveniently this also the easiest way for many adversaries to apply censoring or parental control by using DNS blocking[^24]. The provided DNS servers will give you a different address (than their real one) for some websites (like redirecting thepiratebay to some government website). Such blocking is widely applied worldwide for certain sites[^25].
...
Because it's not encrypted, your ISP and/or any other adversary could still intercept (using a Man-in-the-middle attack[^56]) your request will know and possibly log what your IP was looking for. The same ISP can also tamper with the DNS responses even if you're using a private DNS. Rendering the use of a private DNS service useless.

As a bonus, many devices and apps will use hardcoded DNS servers bypassing any system setting you could set. This is for example the case with most (70%) Smart TVs and a large part (46%) of Game Consoles[^26]. For these devices, you'll have to force them[^27] to stop using their hardcoded DNS service which could make them stop working properly.

A solution to this is to use encrypted DNS using DoH (DNS over HTTPS[^28]), DoT (DNS over TLS[^29]) with a private DNS server (this can be self-hosted locally with a solution like pi-hole[^30], remotely hosted with a solution like nextdns.io or using the solutions provider by your VPN provider or the Tor network). This should prevent your ISP or some middle-man from snooping on your requests ... except it might not.

Unfortunately the TLS protocol used in most HTTPS connections in most Browsers (Chrome/Brave among them) will leak the DNS again through SNI[^31] handshakes (this can be checked here at Cloudflare: <https://www.cloudflare.com/ssl/encrypted-sni/> ). **As of the writing of this guide**, **only Firefox based browsers supports eSNI (encrypted SNI**[^32] **soon to be renamed ECH**[^33] **and soon to be supported by Firefox too**[^34]**) which will encrypt everything end to end (in addition to using a secure private DNS over TLS/HTTPS) and will allow you to actually hide your DNS requests from a third party**[^35]**.** And this option is not enabled by default either so you'll have to enable it yourself:

In addition to limited browser support, only Web Services and CDNs[^36] behind Cloudflare CDN support eSNI at this stage (and will support ECH in the future[^37]). This means that eSNI (or soon ECH) is not supported (as of January 2021) by many mainstream platforms such as:
...
Some countries like Russia[^38] and China[^39] will block eSNI handshakes at network level to allow snooping and prevent bypassing censorship. Meaning you won't be able to establish an HTTPS connection with a service if you don't allow them to see what it was.

Finally, even if you use a custom encrypted DNS server (DoS or DoT) with eSNI support, it might still not be enough as traffic analysis studies[^40] have shown it's still possible to reliably fingerprint and block unwanted requests. Only DNS over Tor was able to demonstrate efficient DNS Privacy in recent studies but even that can still be defeated by other means (see [Your "Anonymized" Tor/VPN traffic:]).

One could also decide to use a Tor Hidden DNS Service or ODoH (Oblivious DNS over HTTPS[^41]) to further increase privacy/anonymity but unfortunately, as far as I know, these services are only provided by Cloudflare as of this writing (<https://blog.cloudflare.com/welcome-hidden-resolver/>, https://blog.cloudflare.com/oblivious-dns/). I **[personally]{.ul}** think these are viable and reasonably secure technical options but there is also a moral choice if you want to use them (despite the risk posed by some researchers[^42]). Here is an illustration showing the current state of DNS privacy:
...
The IMEI (International Mobile Equipment Identity[^43]) and the IMSI (International Mobile Subscriber Identity[^44]) are unique numbers created by mobile phone manufacturers and mobile phone operators.

The IMEI is tied directly to the phone you're using. This number is known and tracked by the mobile phone operators and known by the manufacturers. Every time your phone connects to the mobile network, it will register the IMEI on the network along the IMSI (if a SIM card is inserted but that's not even needed). It's also used by many applications (Banking apps abusing the phone permission on Android for instance[^45]) and smartphone Operating Systems (Android/IOS) for identification of the device[^46]. It is possible but difficult (and not illegal in many jurisdiction[^47]) to change the IMEI on a phone but it's probably easier and cheaper to just find and buy some old (working) Burner phone for a few Euros (this guide is for Germany remember) at a flea market or at some random small shop.
...
-   The smartphone OS makers (Google/Apple for Android/IOs) also keep logs of IMEI/IMSI identifications tied to Google/Apple accounts and which user has been using them. They too can trace back the history of the phone and to which accounts it was tied in the past[^51].

-   Government agencies around the world interested in your phone number can and do use[^52] special devices called "IMSI catchers"[^53] like the Stingray[^54] or more recently the Nyxcell[^55]. These devices are able to impersonate (to spoof) a cell phone Antenna and force a specific IMSI (your phone) to connect to it to access the cell network. Once they do, they will be able to use various MITM[^56] (Man-In-The-Middle Attacks) that will allow them to:
...
For these reasons again, you should not to bring your smartphone with you (even turned off, unless you can remove the battery or are certain it's completely powered off) when you conduct sensitive activities. Or you could consider the use of a faraday cage[^58] bag to store your devices. There are many such faraday "signal blocking" bags available for sale and some of these have been studied[^59] for their effectiveness. If you can't afford such bags, you can probably achieve a "decent result" with one or several sheets of aluminum foil (as shown in the previously linked study).

## Your Wi-Fi or Ethernet MAC address:

The MAC address[^60] is a unique identifier tied to your physical Network Interface (Wired Ethernet or WIFI) and could of course be used to track you if it's not randomized. As it was the case with the IMEI, manufacturers of computers and network cards usually keep logs of their sales (usually including things like: Serial number, IMEI, Mac Addresses, ...) and it's possible again for them to track where and when the computer with the MAC address in question was sold and to whom. Even if you bought it with cash in a supermarket, the supermarket might still have CCTV (or a CCTV just outside that shop) and again the time/date of sale could be used to find out who was there using the Mobile Provider antenna logs at that time (IMEI/IMSI).
...
Operating systems have protections in place to randomize those addresses but are still subject to vulnerabilities[^61].
...
-   IOS/Apple:
    ...
    -   Apple does claim[^62] that they anonymize this data using differential privacy[^63] but you'll have to trust them on that.
...
But that's not all those WiFis access points can do. Recently developed techs could even allow someone to track your movements accurately just based on radio interferences. What this means is that it's possible to track your movement inside a room/building based on the radio signals passing through. This might seem like a tinfoil hat conspiracy theory claim but here are the references[^66] with demonstrations showing this tech in action: <http://rfpose.csail.mit.edu/> and the video here: <https://www.youtube.com/watch?v=HgDdaMy8KNE>
...
Your metadata is all the information about your activities without the actual content of those activities. For instance it's like knowing you had a call from an oncologist before then calling your family and friends successively. You don't know what was said during the conversation but you can guess what it was just from the "metadata"[^67].

This metadata will also often include your location that is being harvested by Smartphones, Operating Systems (Android[^68]/IOS), Browsers, Apps, Websites. Odds are there are several companies knowing exactly where you are at any time[^69] because of your smartphone[^70].

This location data has been used in many judicial cases[^71] already as part of "geofence warrants" [^72] that allows law enforcement to ask companies (such as Google/Apple) a list of all devices present at a certain location at a certain time.
...
The question is: Is there someone somewhere that would possibly have both pieces of information available[^73] for correlation in a convenient database?

Have you heard of Edward Snowden[^74]? Now is the time to google him and read his book[^75]. I recommend reading about XKEYSCORE[^76]'[^77], MUSCULAR[^78] and PRISM[^79].

"We kill people based on Metadata"[^80]
...
All of this data is very likely being transmitted, processed and stored indefinitely (unencrypted[^81]) by various third parties[^82].
...
-   Your Fitness Devices and Apps? (Strava[^83]'[^84], Fitbit[^85], Garmin, Polar[^86], ...)[^87]

-   Your Smart Speaker? (Amazon Alexa[^88], Google Echo, Apple Homepod ...).
...
So when you're going to conduct anonymous or sensitive activities somewhere. Do not take your smart devices with you. Just don't and if you absolutely have to, make sure they're powered off and consider the use of a faraday cage[^89] bag to store your devices. There are many such faraday "signal blocking" bags available for sale and some of these have been studied[^90] for their effectiveness. If you can't afford such bags, you can probably achieve a "decent result" with one or several sheets of aluminum foil (as shown in the previously linked study).
...
-   IPhones and IPads (IOS 13 and above)[^91]'[^92].

-   Samsung Phones (Android 10 and above)[^93].

-   MacBooks (MacOS 10.15 and above)[^94].

Such devices will continue to broadcast identity information to nearby devices even when offline using Bluetooth Low-Energy[^95]. They don't have access to the devices directly (which are not connected to the internet) but instead use BLE to find them through other nearby devices[^96].
...
For this reason again, you should probably not to bring your smartphone with you (even turned off, unless you can remove the battery or are certain it's completely powered off) when you conduct sensitive activities. Or you could consider the use of a faraday cage[^97] bag to store your devices. There are many such faraday "signal blocking" bags available for sale and some of these have been studied[^98] for their effectiveness. If you can't afford such bags, you can probably achieve a "decent result" with one or several sheets of aluminum foil (as shown in the previously linked study).

## Your RFID enabled devices:

RFID stands for Radio-frequency identification[^99], it's the technology used for instance for contactless payments and various identification systems. Of course your smartphone is among those devices and has RFID contactless payment capabilities through NFC[^100]. As with everything else, such capabilities can be used for tracking by various actors.
...
Such watermarks can be inserted by various products[^106]'[^107]'[^108]'[^109] using Steganography[^110] and can resist compression[^111] and re-encoding[^112]'[^113].

These watermarks are not easily detectable and could allow identification of the source despite all efforts.

In addition to watermarks, the camera used for filming (and therefore the device used for filming) a video can also be identified using various techniques such as lens identification[^114] which could lead to de-anonymization.
...
Tor and VPNs are not silver bullets. Many advanced techniques have been developed and studied to de-anonymize encrypted traffic over the years[^115]. Most of those techniques are Correlation attacks that will correlate your network traffic in one way or another to logs or datasets. Here are some classic examples:

-   Correlation Fingerprinting Attack: As illustrated (simplified) below, this attack will fingerprint[^116] your encrypted traffic (like the websites you visited) just based on the analysis of your encrypted traffic (without decrypting it). It's able to do so with a whopping 96% success rate. Such fingerprinting can be used by an adversary that has access to your source network to figure out some of your encrypted activity (such as which websites you visited).

![][21]

-   Correlation Timing Attacks: As illustrated (simplified) below, an adversary that has access to network connection logs (IP or DNS for instance, remember that most VPN servers and most Tor nodes are known and publicly listed) at the source and at the destination could correlate the timings to de-anonymize you without requiring any access to the Tor or VPN network in between. A real use case of this technique was done by the FBI in 2013 to de-anonymize[^117] a bomb threat hoax at Harvard University.
...
Be aware again that this might not be enough against a motivated global adversary[^118] with wide access to global mass surveillance (remember XKEYSCORE, MUSCULAR and PRISM). Such adversary might have access to logs no matter where you are and could use those to de-anonymize you.
...
Lastly, do remember that using Tor in itself can already be considered a suspicious activity[^119] and its use could be considered malicious by some[^120].
...
Contrary to popular belief, Crypto transactions (such as Bitcoin and Ethereum) are not anonymous[^121]. Most crypto currencies can be tracked accurately through various methods[^122].

The main issue is not setting up a random Crypto wallet to receive some currency behind a VPN/Tor address (at this point, the wallet is anonymous). The issue is mainly when you want to convert Fiat money (Euros, Dollars ...) to Crypto and then when you want to cash in your Crypto. You'll have few realistic options but to transfer those to an exchange (such as Coinbase/Kraken/Bitstamp/Binance). Those exchanges have known wallet addresses and will keep detailed logs (due to KYC[^123] financial regulations) and can then trace back those crypto transactions to you using the financial system.

There are some crypto currencies with privacy in mind like Monero but even those have some and warnings to consider[^124]'[^125].

Even if you use Mixers or Tumblers (services that specialize in anonymizing crypto currencies by "mixing them"), keep in mind this is only obfuscation and not actual anonymity[^126].
...
So you're using Tor Browser or Brave Browser with a Tor Tab. You could be using those over a VPN for added security. But you should keep in mind that there are exploits[^127] (hacks) that could be known by an adversary (but unknown to the App/Browser provider). Such exploits could be used to compromise your system and reveal details to de-anonymize you such as your IP address or other details.

A real use case of this technique was the Freedom Hosting[^128] case in 2013 where the FBI inserted malware[^129] using a Firefox browser exploit on a Tor website. This exploit allowed them to reveal details of some users. More recently, there was the notable SolarWinds[^130] hack that breached several US government institutions by inserting malware into an official software update server.
...
To reflect these recommendations, this guide will therefore later guide you in the use of Virtualization so that even if your Browser/Apps get compromised by a skilled adversary, that adversary will find himself stuck in a sandbox[^131] without being able to access identifying information or compromise your system.

## Your CPU:

All modern CPUs[^132] are now integrating hidden management platforms such as the now infamous Intel Management Engine[^133] and the AMD Platform Security Processor[^134].
...
These have already been affected by several security vulnerabilities in the past[^135] that allowed malware to gain control of target systems. These are also accused by many privacy actors including the EFF and Libreboot of being a backdoor into any system[^136].

There are some not so easy ways[^137] to disable the Intel IME on some CPUs and you should do so if you can. For some AMD laptops, you can disable it within the BIOS settings by disabling PSP.

If you're feeling a bit more adventurous, you could install your own BIOS using Libreboot[^138] or Coreboot[^205] if your laptop supports it.
...
These have been used since at least since 2008 using an attack called "Jasager"[^139] and can be done by anyone using self-built tools or using commercially available devices such as Wi-Fi Pineapple[^140].
...
These devices can fit in a small bag and can take over the Wi-Fi environment of any place within their range. For instance a Bar/Restaurant/Café/Hotel Lobby. These devices can force Wi-Fi clients to disconnect from their current Wi-Fi (using de-authentication, disassociation attacks[^141]) while spoofing the normal Wi-Fi networks at the same location. They will continue to perform this attack until your computer or yourself decides to try to connect to the rogue AP.

These devices can then mimic a captive portal[^142] with the exact same layout as the Wi-Fi you're trying to access (for instance an Airport Wi-Fi registration portal). Or they could just give you open access internet that they'll themselves get from the same place.
...
There are readily available commercial and cheap "badUSB" [^143]devices that can take deploy malware, log your typing, geolocate you, listen to you or gain control of your laptop just by plugging them in. Here are some examples that you can already buy yourself.
...
Such devices can be implanted anywhere (charging cable, mouse, keyboard, USB key ...) by an adversary and can be used to track you or compromise your computer or smartphone. The most notable example of such attacks is probably Stuxnet[^144] in 2005.

While you could inspect an USB key physically, Scan it with various utilities, check the various components to see if they're genuine, you will most likely never be able to discover complex malware embedded in genuine parts of a genuine USB key by a skilled adversary without advanced forensics equipment[^145].
...
There are specialized commercial forensics solutions available (Magnet Axiom[^146], Cellebrite Cloud[^147]) that will help an adversary analyze your cloud data with ease.
...
The digital fingerprint is the way you write[^148], the way you behave[^149]. The way you click. The way you browse. The fonts you use on your browser[^150]. Fingerprinting is being used to guess who someone is by the way that user is behaving. You might be using specific pedantic words or making specific spelling mistakes that could give you away using a simple Google search for similar features because you typed in a similar way on some Reddit post 5 years ago using a not so anonymous Reddit account.
...
This technology is also widely used in CAPTCHAS[^260] services to verify that you are "human" and can be used to fingerprint a user.
...
A real use and well-documented case of this was the arrest of the hacker Jeremy Hammond[^151] who shared over time several details about his past and was later discovered.

There are also a few cases involving OSINT at Bellingcat[^152].Have a look at their very informative toolkit here: <https://bit.ly/bcattools>
...
Your Browser and Device Fingerprints[^270] are set of properties/capabilities of your System/Browser. These are used on most websites for invisible user tracking but also to adapt the website user experience depending on their browser. For instance websites will be able to provide a "mobile experience" if you're using a mobile browser or propose a specific language/geographic version depending on your fingerprint. Most of those techniques work with recent Browsers like Chromium[^153] based browsers (such as Chrome) or Firefox[^154] unless taking special measures.
...
Companies like Facebook have used advanced face recognition for years[^155]'[^156] and have been using other means (Satellite imagery) to create maps of "people" around the world[^157].
...
Even if you're not looking at the camera, they can still figure out who you are[^160], make out your emotions[^161] and probably guess your political affiliation[^162].
...
-   Even if you never made a profile on those platforms, you still have one without even knowing it[^163]'[^164]'[^165]'[^166]'[^167].
...
Governments already know who you are because they have your ID/Passport/Driving License pictures and often added biometrics (Fingerprints) in their database. Those same governments are integrating those technologies (often provided by private companies such as the Israeli AnyVision[^168]) in their CCTV networks to look for "persons of interest"[^169]. And some heavily surveilled states like China have implemented widespread use of Facial Recognition for various purposes[^170] including possibly identifying ethnic minorities[^171]. A simple face recognition error by some algorithm can ruin your life[^172].
...
Same goes with fingerprint authentication being mainstreamed by many smartphone makers to authenticate yourself. A simple picture where your fingers appear can be used to de-anonymize you[^173].

We can safely imagine a near future where you won't be able to create accounts or sign-in anywhere without providing unique biometrics (A good time to re-watch Gattaca[^174], Person of Interest[^175] and Minority Report[^176]). And you can safely imagine how useful these large biometrics databases could be to some interested third parties.

In addition, all this information can also be used against you (if you are already de-anonymized) using deepfake[^177] by crafting false information (Pictures, Videos, Voice Recordings...) and have already been used for such purposes[^178].

At this time, there are a few steps[^179] you can use to mitigate (and only mitigate) face recognition when conducting sensitive activities where CCTV might be present:

-   Wear a facemask as they have been proven to defeat some face recognition technologies[^180].
...
Phishing[^181] is a type of attack where an adversary could try to extract information from you by pretending to be something/someone else.

A typical case is an adversary using a man-in-the-middle[^56] attack or a falsified e-mail/call to ask your credential for a service. This can be your e-mail or your financial services for example.
...
-   If you absolutely want system wide plausible deniability[^216]'[^196] despite the risks[^197]'[^218].
...
**Always use passphrases instead of passwords and use a different one for each service. Do not make it easy for an adversary to access all your information because you used the same password everywhere**[^207]**.**
...
For additional safety (if you absolutely have to take your smartphone along with you or if you want to store your Burner phone), you could consider the use of a faraday cage[^198] bag to store your devices. There are many such faraday "signal blocking" bags available for sale and some of these have been studied[^199] for their effectiveness. If you can't afford such bags, you can probably achieve a "decent result" with one or several sheets of aluminum foil (as shown in the previously linked study).
...
This is the hardest part of the whole guide. It's a SPOF (Single Point of Failure). The places where you can still buy prepaid SIM cards without ID registration are getting more and more limited due to various KYC type regulations[^200].
...
**DISCLAIMER: Do not attempt this until you are done setting up a secure environment according to one of the selected routes. This step will require online access and should only be done from a secure network. Do not do this from any known/unsecure environment. Skip this until you have finished one of the routes.**
...
There are some forums and subreddits (like r/phoneverification/) where users will offer the service of receiving such SMS messages for you for a small fee (using PayPal or some crypto payment). Unfortunately these are full of scammer and very risky in terms of anonymity.

To this date, I do not know any reputable service that would offer this service and accept cash payments (by post for instance) like VPN providers. But there is one service I'm aware of that provides online phone numbers and does accept Monero which could be reasonably anonymous (yet less recommended than that physical way in the previous chapter).

If you are willing to take that route, you might consider:

-   <https://dtmf.io/> which even provides an onion address for direct access through the Tor Network at <http://dtmfiovjh42uviqez6qn75igbagtiyo724hy3rdxm77dy2m5tt7lbaqd.onion/>
...
Some might be very efficient[^201] but many are gimmicky gadgets that offer no real protection[^202].
...
TAILS[^203] stands for **The Amnesic Incognito Live System.** . It's a bootable Live Operating System running from a USB key that is designed for leaving no traces and forcing all connections through the Tor network.
...
-   **Do not use hidden volumes on SSD drives as this is not supported/recommended by Veracrypt**[^204]**.**
...
If you can afford it, you might consider getting a Purism Librem laptop (<https://puri.sm>) or System76 laptops (<https://system76.com/>) while using Coreboot[^205] (where Intel IME is disabled from factory).
...
-   AMD laptops could be more interesting as some provide the ability to disable AMD PSP (the AMD equivalent of Intel IME) from the BIOS/UEFI settings by default. However if you're going for the Qubes Route consider Intel as they do not support AMD with their anti-evil-maid system[^206].
...
-   Enable BIOS/UEFI password and use a long passphrase[^207] instead of a password if you can and make sure this password is required for:
...
-   Disable Secure Boot if you intend to use QubesOS as they do not support it out of the box[^208]. Keep it on if you intend to use Linux/Windows.
...
So what is Secure Boot[^209]? In short, it's a UEFI security feature designed to prevent your computer from booting an operating system from which the bootloader was not signed by specific keys stored in the UEFI firmware of your laptop.

Basically when the Operating Systems (or the Bootloader[^210]) supports it, you can store the keys of your bootloader in your UEFI firmware and this will prevent booting up any unauthorized Operating System (such as a live OS USB or anything similar).
...
Here is a good cheap method to make your laptop tamper proof using Nail Polish (with glitter) <https://mullvad.net/en/help/how-tamper-protect-laptop/> [^211] (with pictures).
...
This route will make extensive use of Virtual Machines[^212], they'll require a host OS to run the Virtualization software. You have 3 recommended choices in this part of the guide:
...
Linux is also not necessarily the best choice for privacy depending on your threat model. This is because using Windows will allow us to use Plausible Deniability[^216] at the OS level. Windows is also unfortunately at the same time a privacy nightmare[^217] but is the only (convenient) option for using OS wide plausible deniability. Windows telemetry and telemetry blocking is also widely documented which should mitigate many issues.

So what is Plausible Deniability? It's the ability for you to cooperate with an adversary requesting access to your device/data without revealing your true secret. All this using Deniable Encryption[^218].

A soft lawful adversary could ask for your encrypted laptop password. At first you could refuse to give out any password (using your "right to remain silent", "right not to incriminate yourself") but some countries are implementing laws[^219]'[^220] to exempt this from such rights (because terrorists and "think of the children"). In that case you might have to reveal the password or maybe face jail time in contempt of court. This is where plausible deniability will comes into play.
...
Cold Boot attacks[^223] are trickier than the Evil Maid Attack but can be part of an Evil Maid attack as it requires an adversary to come into possession of your laptop while you're actively using your device or shortly afterward.

The idea is rather simple, as shown in this video[^224], an adversary could theoretically quickly boot your device on a special USB key that would copy the content of the RAM (the memory) of the device after you shut it down. If the USB ports are disabled or if they feel like they need more time, they could open it and "cool down" the memory using a spray or other chemicals (liquid nitrogen for instance) preventing the memory decaying. They could then be able to copy its content for analysis. This memory dump could contain the key to decrypt your device. We will later apply a few principles to mitigate these.

In the case of Plausible Deniability. There have been some forensics studies[^225] about technically proving the presence of the hidden data with a simple forensic examination (without a Cold Boot/Evil Maid Attack) but these have been contested by other studies[^226] and by the maintainer of Veracrypt[^227] so I wouldn't worry too much about those yet.
...
-   Windows ShellBags that are stored within the Windows Registry silently storing various histories of accessed volumes/files/folders[^229].

-   Windows Indexing keeping traces of the files present in your User folder by default[^230].

-   Recent lists (aka Jump Lists) in Windows and various apps keeping traces of recently accessed documents[^231].
...
-   Gatekeeper[^232] and XProtect keeping track of your download history in a local database and file attributes.
...
Forensics could[^226]'[^229] use all those leaks[^236] to prove the existence of hidden data and defeat your attempts at using plausible deniability and to find out about your various sensitive activities.
...
Veracrypt[^233] is the software I will recommend for full disk encryption, file encryption and plausible deniability. It is a fork of the well-known but deprecated and unmaintained TrueCrypt. It can be used for
...
It is to my knowledge the only (convenient and usable by anyone) free, open-source and openly audited[^234] encryption software that also provides plausible deniability for general use and it works with Windows Home Edition.
...
-   Encrypt the memory with a Veracrypt option[^235] (settings \> performance/driver options \> encrypt RAM) at a cost of 5-15% performance. This will also disable hibernation (which doesn't clear the key when hibernating) and instead encrypt the memory altogether to mitigate some cold-boot attacks.
...
-   Enable the Veracrypt option to mount volumes as removable volumes (Settings \> Preferences \> Mount volume as removable media). This will prevent Windows from writing some logs about your mounts in the Event logs[^236] and prevent some local data leaks.
...
-   Route C: Plausible Deniability Encryption of your Operating system (you'll have a "hidden OS" and a "decoy OS" running on the laptop):
    ...
    -   Cons:
        ...
        -   No use with an SSD drive due to requirement of disabling Trim[^237] Operations[^238]. This will severely degrade the performance/health of your SSD drive over time.
...
-   Use a strong passphrase[^207] for the Hidden Volume (obviously a different one than the one for the Outer Volume).
...
-   To rescue disk[^245] or not rescue disk, well that's up to you. I recommend making one (just in case), just make sure to store it outside your encrypted drive (USB key for instance, or wait and see the end of this guide for guidance on safe backups). This rescue disk will not store your passphrase and you'll still need it to use it.
...
In this route, we will make extensive use of the free Oracle Virtualbox[^246] software. This is a virtualization software in which you can create Virtual Machines that emulate a computer running a specific OS (if you want to use something else like Xen, Qemu, KVM or VMWARE, feel free to do so but this part of the guide covers Virtualbox only for convenience).

You should be aware that Virtualbox is not the best virtualization software in terms of security and some of the reported issues[^247] have not be completely fixed to this date[^248] and if you're using Linux with a bit more technical skills, you should consider using KVM instead by following the guide available at Whonix here <https://www.whonix.org/wiki/KVM>
...
This route will use Virtualization and Whonix[^249] as part of the anonymization process. Whonix is a Linux distribution composed of two Virtual Machines:
...
Later on you will create and run several Virtual Machines within Virtualbox for your sensitive activities. Virtualbox provides a feature called "Snapshots"[^250] that allow for saving the state of a VM at any point in time. If for any reason later you want to go back to that state, you can restore that snapshot at any moment.
...
You will need something to store your data (logins/passwords, identities and TOTP[^254] information).

For this purpose I strongly recommend KeePassXC because of their TOTP feature. This is the ability to create entries for 2FA[^255] authentication with the authenticator feature.
...
In itself Qubes is the best option in this guide for people who are more comfortable with Linux and tech in general. But it has some downsides such as the lack of OS wide plausible deniability and its hardware requirements and hardware compatibilities. While you can run this on 4GB of RAM as per their requirements[^256], the recommended RAM is 16GB. I would advise against using Qubes OS if you have less than 8GB of RAM.
...
**From your cash-paid VPN provider and using a Tor browser (be careful not to use any Clearnet Browser for this, use Tor Browser or Brave with a Tor Tab, double or triple-check),** download the necessary OpenVPN configuration files for Linux.
...
You will need something to store your data (logins/passwords, identities and TOTP[^258] information).

For this purpose I strongly recommend KeePassXC because of their TOTP feature. This is the ability to create entries for 2FA[^259] authentication with the authenticator feature.
...
Captcha[^260] stands for "Completely Automated Public Turing test to tell Computers and Humans Apart" are Turing tests[^261] puzzles you need to complete before accessing a form/website. You'll mostly encounter those provided by Google (reCaptcha service[^262]) and Cloudflare (hCaptcha[^263]). hCaptcha is used on 15% of the internet by their own metrics[^264].
...
Most platforms (including the privacy oriented ones such as Signal/Telegram/ProtonMail will require a phone number to register and most countries now make it mandatory to submit a proof of ID to register[^269].
...
Browser and Device[^270] Fingerprinting are usually integrated into the Captcha services but also in other various services.

Many platforms (like Google[^271]) will check your browser for various capabilities and settings and block Browsers they don't like. This is one of the reasons I recommend using Brave Browser over Tor Browser within your VM.
...
This is the part where you should watch the documentary "The Social Dilemma"[^272] on Netflix as they cover this topic much better than anyone else IMHO.
...
This is for instance now the case on YouTube for some European Users[^273].
...
This option is often presented as the "default one", hiding away the "Sign-in with e-mail and password" with clever Dark Patterns[^274] and unfortunately sometimes required.
...
Some platforms/apps will require you to take a live picture of yourself either doing something (a wink, holding an arm up ...) or showing a custom piece of information (a hand written text, a passport or ID) within the picture. Sometimes the platform/app will require several pictures to increase their certainty.![][37]

This guide won't cover this one (yet) as it's mainly used on financial platforms (that will be able to identify you with other means anyway) and some dating apps like Tinder[^275]. Unfortunately this method is now also sometimes being used on Facebook[^276] and Instagram as part of their verification methods (tho I didn't face it yet so far).
...
Recently even platforms such as PornHub decided to implement similar measures in the future[^277].
...
So what? Well instead of not trusting anyone or anything, I would advise to **"Trust but verify"**[^278] instead.
...
-   **You are fully done and preparing one of the routes:**
    ...
    -   **You are running TAILS or a safe VM within Tails (HiddenVM).**

    -   **You are running a Whonix Workstation VM.**

    -   **You are running a Windows/Linux/MacOS VM with traffic correctly routed through Whonix Gateway.**

    -   **You are running an AppVM Qube within Qubes OS with all traffic routed through Whonix.**

**Again, it is crucially important to understand that you will be unable to create most accounts without a valid phone number. Therefore, most of your anonymity on mainstream platforms depends on the anonymity of your phone number and/or the burner phone with its pre-paid SIM card (if you use one). If your phone number is not anonymous or your burner phone can be traced back to you then you can be de-anonymized. If you can't get this anonymous phone number and/or a physical SIM with a Burner phone, then you'll have to restrict yourself to platforms not asking for phone number verification.**

**Do not to bring your smartphone with you (even turned off, unless you can remove the battery or are certain it's completely powered off) when you conduct sensitive activities. Or you could consider the use of a faraday cage**[^279] **bag to store your devices. There are many such faraday "signal blocking" bags available for sale and some of these have been studied**[^280] **for their effectiveness. If you can't afford such bags, you can probably achieve a "decent result" with one or several sheets of aluminum foil (as shown in the previously linked study).**

### Creating new identities:

This is the fun part where you will now create your identities from thin air. These identities do not exist but should be plausible and look "organic". They should ideally have a story ... a "legend" (yes this is actually the official term for this[^281]).
...
-   Brave Browser (Chromium based) with a Private Tor Tab has (IMHO) a better acceptance level than Tor Browser (Firefox based). You will have less issues with captchas and online platforms[^271] if you use Brave than if you use Tor Browser (feel free to try this yourself).
...
-   Remember, **always use passphrases instead of passwords and use a different one for each service. Do not make it easy for an adversary to access all your information because you used the same password everywhere**[^207]**.**
...
Windows Event logs will keep many various pieces of information that could contain traces of your activities such as the devices that were mounted (including Veracrypt NTFS volumes for instance[^236]), your network connections, app crash information and various errors. It's always best to clean those up regularly. Do not do this on the Decoy OS.
...
As explained earlier, Shellbags are basically histories of accessed volumes/files on your computer. Remember that shellbags are very good sources of information for forensics[^229] and you need to clean those. Especially if you mounted any "hidden volume" anywhere. Again you shouldn't do this on the Decoy OS.
...
-   Download and install PrivaZer from <https://privazer.com/en/download.php>
    ...
    -   (if you did select Free Space cleaning) Within Clean Options **(Be careful with this option as it will erase all the free space on the selected partition, especially if you are running the decoy OS. Do not erase the free space or anything else on the second partition as you risk destroying your Hidden OS)**
        ...
        -   If you have an SSD drive:
            ...
            -   Secure Overwriting Tab: Personally I would just pick Normal Deletion + Trim (Trim itself should be enough[^240]). Secure Deletion with Trim[^237] (1 pass) might be redundant and overkill here if you intend to overwrite the free space anyway.
...
DuckDuckGo does not store cached version of pages[^318] and will instead forward you to a Google/Bing cached version if available.

In addition, DuckDuckGo source most of their searches from Bing (and not google)[^319] and therefore removing the content from Bing should in time have it removed it from DuckDuckGo too.
...
A commonly used technique which combines the idea of a Dead Drop[^320] and Secure Communication Obfuscation[^321] through Steganography[^322] and/or Kleptography[^323] and has many names such as Koalang[^324] or "Talking Around" or even "Social Steganography". This technique is very old and still widely used nowadays by teenagers to bypass parental control. It's hiding in plain sight.
...
If you want to give a hand to users facing censorship and oppression accessing the Tor Network, consider using <https://addons.mozilla.org/en-US/firefox/addon/torproject-snowflake/> [^325]
...
As written earlier in this guide and as noted by Privacytools.io[^326], Windows 10 is a privacy nightmare. And disabling everything during and after the installation using the settings available to you is not enough. The amount of telemetry data collected by Microsoft is staggering and could defeat your attempts at keeping secrets. You will need to download and use a couple utilities to (hopefully) force Windows 10 into not sending data back to Microsoft.
...
-   The Windows USN journal which stores plenty of information[^328].

-   The Windows System Resource Usage Monitor (SRUM)[^329].

-   Various histories of various programs (such as the recent lists).

-   Various logs

-   The free (unallocated) space of your hard drive[^330].
...
There are several utilities that are recommend (like the old unmaintained DBAN[^331]) or System Rescue CD (<https://www.system-rescue.org/>) for this but personally, I will recommend the use of ShredOS.
...
Some might be very efficient[^332] but many are gimmicky gadgets.
...
**Use the manufacturer provided tools to check and perform these operations** ... It's pretty much the only way to be sure it's not only supported but actually works. Some utilities can tell you if it's supported or not like CrystalDiskInfo[^333] but won't actually check if it's working. See [Appendix J: Manufacturer tools for Wiping HDD and SSD drives:]
...
[^13]: Tor Project, Who Uses Tor <https://2019.www.torproject.org/about/torusers.html.en>

[^14]: Whonix Documentation, The importance of Anonymity <https://www.whonix.org/wiki/Anonymity>

[^15]: Wikipedia, IANAL <https://en.wikipedia.org/wiki/IANAL>

[^16]: Wikipedia, Trust but verify <https://en.wikipedia.org/wiki/Trust,_but_verify>

[^17]: English translation of German Telemedia Act <https://www.huntonprivacyblog.com/wp-content/uploads/sites/28/2016/02/Telemedia_Act__TMA_.pdf>. Section 13, Article 6, "The service provider must enable the use of Telemedia and payment for them to occur anonymously or via a pseudonym where this is technically possible and reasonable. The recipient of the service is to be informed about this possibility. ".

[^18]: Wikipedia, Don't be evil <https://en.wikipedia.org/wiki/Don%27t_be_evil>

[^19]: Wikipedia, IP Address, <https://en.wikipedia.org/wiki/IP_address>

[^20]: Wikipedia; Data Retention <https://en.wikipedia.org/wiki/Data_retention>

[^21]: Wikipedia, Tor Anonymity Network <https://en.wikipedia.org/wiki/Tor_(anonymity_network)>

[^22]: Wikipedia, VPN <https://en.wikipedia.org/wiki/Virtual_private_network>

[^23]: Wikipedia, DNS <https://en.wikipedia.org/wiki/Domain_Name_System>

[^24]: Wikipedia, DNS Blocking <https://en.wikipedia.org/wiki/DNS_blocking>

[^25]: CensoredPlanet <https://censoredplanet.org/>

[^26]: ArXiv, Characterizing Smart Home IoT Traffic in the Wild <https://arxiv.org/pdf/2001.08288.pdf>

[^27]: Labzilla.io, Your Smart TV is probably ignoring your Pi-Hole <https://labzilla.io/blog/force-dns-pihole>

[^28]: Wikipedia, DNS over HTTPS: <https://en.wikipedia.org/wiki/DNS_over_HTTPS>

[^29]: Wikipedia, DNS over TLS, <https://en.wikipedia.org/wiki/DNS_over_TLS>

[^30]: Wikipedia, Pi-Hole <https://en.wikipedia.org/wiki/Pi-hole>

[^31]: Wikipedia, SNI <https://en.wikipedia.org/wiki/Server_Name_Indication>

[^32]: Wikipedia, eSNI <https://en.wikipedia.org/wiki/Server_Name_Indication#Encrypted_Client_Hello>

[^33]: Wikipedia, ECH, <https://en.wikipedia.org/wiki/Server_Name_Indication#Encrypted_Client_Hello>

[^34]: Mozilla Blog, Encrypted Client Hello: the future of ESNI in Firefox <https://blog.mozilla.org/security/2021/01/07/encrypted-client-hello-the-future-of-esni-in-firefox/>

[^35]: Usenix.org, On the Importance of Encrypted-SNI (ESNI) to Censorship Circumvention <https://www.usenix.org/system/files/foci19-paper_chai_0.pdf>

[^36]: Wikipedia, CDN <https://en.wikipedia.org/wiki/Content_delivery_network>

[^37]: Cloudflare, Good-bye ESNI, hello ECH! <https://blog.cloudflare.com/encrypted-client-hello/>

[^38]: ZDNET, Russia wants to ban the use of secure protocols such as TLS 1.3, DoH, DoT, ESNI <https://www.zdnet.com/article/russia-wants-to-ban-the-use-of-secure-protocols-such-as-tls-1-3-doh-dot-esni/>

[^39]: ZDNET, China is now blocking all encrypted HTTPS traffic that uses TLS 1.3 and ESNI <https://www.zdnet.com/article/china-is-now-blocking-all-encrypted-https-traffic-using-tls-1-3-and-esni/>

[^40]: KUL, Encrypted DNS=⇒Privacy? A Traffic Analysis Perspective <https://www.esat.kuleuven.be/cosic/publications/article-3153.pdf>

[^41]: ResearhGate, Oblivious DNS: Practical Privacy for DNS Queries <https://www.researchgate.net/publication/332893422_Oblivious_DNS_Practical_Privacy_for_DNS_Queries>

[^42]: Nymity.ch, The Effect of DNS on Tor's Anonymity <https://nymity.ch/tor-dns/>

[^43]: Wikipedia, IMEI <https://en.wikipedia.org/wiki/International_Mobile_Equipment_Identity>

[^44]: Wikipedia, IMSI <https://en.wikipedia.org/wiki/International_mobile_subscriber_identity>

[^45]: Android Documentation, Device Identifiers <https://source.android.com/devices/tech/config/device-identifiers>

[^46]: Google Privacy Policy, Look for IMEI <https://policies.google.com/privacy/embedded?hl=en-US>

[^47]: Wikipedia, IMEI and the Law <https://en.wikipedia.org/wiki/International_Mobile_Equipment_Identity#IMEI_and_the_law>

[^48]: Bellingcat, The GRU Globetrotters: Mission London <https://www.bellingcat.com/news/uk-and-europe/2019/06/28/the-gru-globetrotters-mission-london/>

[^49]: Bellingcat,\"V\" For "Vympel": FSB's Secretive Department "V" Behind Assassination Of Georgian Asylum Seeker In Germany <https://www.bellingcat.com/news/uk-and-europe/2020/02/17/v-like-vympel-fsbs-secretive-department-v-behind-assassination-of-zelimkhan-khangoshvili/>

[^50]: Wikipedia, CCTV <https://en.wikipedia.org/wiki/Closed-circuit_television>

[^51]: Apple, Transparency Report, Device Requests <https://www.apple.com/legal/transparency/device-requests.html>

[^52]: The Intercept, How Cops Can Secretly Track Your Phone <https://theintercept.com/2020/07/31/protests-surveillance-stingrays-dirtboxes-phone-tracking/>

[^53]: Wikipedia, IMSI Catcher <https://en.wikipedia.org/wiki/IMSI-catcher>

[^54]: Wikipedia, Stingray <https://en.wikipedia.org/wiki/Stingray_phone_tracker>

[^55]: Gizmodo, Cops Turn to Canadian Phone-Tracking Firm After Infamous \'Stingrays\' Become \'Obsolete\' <https://gizmodo.com/american-cops-turns-to-canadian-phone-tracking-firm-aft-1845442778>

[^56]: Wikipedia, MITM <https://en.wikipedia.org/wiki/Man-in-the-middle_attack>

[^57]: Purism, Librem 5 <https://shop.puri.sm/shop/librem-5/>

[^58]: Wikipedia, Faraday Cage, <https://en.wikipedia.org/wiki/Faraday_cage>

[^59]: Edith Cowan University, A forensic examination of several mobile device Faraday bags & materials to test their effectiveness materials to test their effectiveness <https://ro.ecu.edu.au/cgi/viewcontent.cgi?article=1165&context=adf>

[^60]: Wikipedia, MAC Address <https://en.wikipedia.org/wiki/MAC_address>

[^61]: ResearchGate, Tracking Anonymized Bluetooth Devices <https://www.researchgate.net/publication/334590931_Tracking_Anonymized_Bluetooth_Devices/fulltext/5d3308db92851cd04675a469/Tracking-Anonymized-Bluetooth-Devices.pdf>

[^62]: Apple, Differential Privacy White Paper <https://www.apple.com/privacy/docs/Differential_Privacy_Overview.pdf>

[^63]: Wikipedia, Differential Privacy <https://en.wikipedia.org/wiki/Differential_privacy>

[^64]: Google Android Help, Android Location Services <https://support.google.com/accounts/answer/3467281?hl=en>

[^65]: Apple Support, Location Services and Privacy <https://support.apple.com/en-us/HT207056>

[^66]: State University of New York, Towards 3D Human Pose Construction Using Wi-Fi <https://cse.buffalo.edu/~lusu/papers/MobiCom2020.pdf>

[^67]: Using Metadata to find Paul Revere (<https://kieranhealy.org/blog/archives/2013/06/09/using-metadata-to-find-paul-revere/>)

[^68]: Wikipedia, Google SensorVault, <https://en.wikipedia.org/wiki/Sensorvault>

[^69]: NRKBeta, My Phone Was Spying on Me, so I Tracked Down the Surveillants <https://nrkbeta.no/2020/12/03/my-phone-was-spying-on-me-so-i-tracked-down-the-surveillants/>

[^70]: New York Times <https://www.nytimes.com/interactive/2019/12/19/opinion/location-tracking-cell-phone.html>

[^71]: Sophos, Google data puts innocent man at the scene of a crime <https://nakedsecurity.sophos.com/2020/03/10/google-data-puts-innocent-man-at-the-scene-of-a-crime/>

[^72]: Wikipedia, Geofence Warrant <https://en.wikipedia.org/wiki/Geo-fence_warrant>

[^73]: Wikipedia, Room 641A <https://en.wikipedia.org/wiki/Room_641A>

[^74]: Wikipedia, Edward Snowden <https://en.wikipedia.org/wiki/Edward_Snowden>

[^75]: Wikipedia, Permanent Record <https://en.wikipedia.org/wiki/Permanent_Record_(autobiography)>

[^76]: Wikipedia, XKEYSCORE <https://en.wikipedia.org/wiki/XKeyscore>

[^77]: ElectroSpaces, Danish military intelligence uses XKEYSCORE to tap cables in cooperation with the NSA <https://www.electrospaces.net/2020/10/danish-military-intelligence-uses.html>

[^78]: Wikipedia, MUSCULAR <https://en.m.wikipedia.org/wiki/MUSCULAR_(surveillance_program)>

[^79]: Wikipedia, PRISM <https://en.wikipedia.org/wiki/PRISM_(surveillance_program)>

[^80]: Justsecurity, General Hayden <https://www.justsecurity.org/10318/video-clip-director-nsa-cia-we-kill-people-based-metadata/>

[^81]: Reuters, Exclusive: Apple dropped plan for encrypting backups after FBI complained -- sources <https://www.reuters.com/article/us-apple-fbi-icloud-exclusive-idUSKBN1ZK1CT>

[^82]: ZDnet, I asked Apple for all my data. Here\'s what was sent back <https://www.zdnet.com/article/apple-data-collection-stored-request/>

[^83]: Wired, The Strava Heat Map and the End of Secrets <https://www.wired.com/story/strava-heat-map-military-bases-fitness-trackers-privacy/>

[^84]: Bellingcat, How to Use and Interpret Data from Strava\'s Activity Map <https://www.bellingcat.com/resources/how-tos/2018/01/29/strava-interpretation-guide/>

[^85]: The Guardian, Fitness tracking app Strava gives away location of secret US army bases <https://www.theguardian.com/world/2018/jan/28/fitness-tracking-app-gives-away-location-of-secret-us-army-bases>

[^86]: Telegraph, Running app reveals locations of secret service agents in MI6 and GCHQ <https://www.telegraph.co.uk/technology/2018/07/08/running-app-exposes-mi6-gchq-workers-whereabouts/>

[^87]: De Correspondent, Here's how we found the names and addresses of soldiers and secret agents using a simple fitness app <https://decorrespondent.nl/8481/heres-how-we-found-the-names-and-addresses-of-soldiers-and-secret-agents-using-a-simple-fitness-app/412999257-6756ba27>

[^88]: Washington Post, Alexa has been eavesdropping on you this whole time <https://www.washingtonpost.com/technology/2019/05/06/alexa-has-been-eavesdropping-you-this-whole-time/?utm_term=.8514f3a17b1c&itid=lk_interstitial_manual_59>

[^89]: Wikipedia, Faraday Cage, <https://en.wikipedia.org/wiki/Faraday_cage>

[^90]: Edith Cowan University, A forensic examination of several mobile device Faraday bags & materials to test their effectiveness materials to test their effectiveness <https://ro.ecu.edu.au/cgi/viewcontent.cgi?article=1165&context=adf>

[^91]: CryptoEngineering, How does Apple (privately) find your offline devices? <https://blog.cryptographyengineering.com/2019/06/05/how-does-apple-privately-find-your-offline-devices/>

[^92]: Apple Support <https://support.apple.com/en-us/HT210515>

[^93]: XDA, Samsung's Find My Mobile app can locate Galaxy devices even when they're offline <https://www.xda-developers.com/samsung-find-my-mobile-app-locate-galaxy-devices-offline/>

[^94]: Apple Support, If your Mac is lost or stolen <https://support.apple.com/en-us/HT204756>

[^95]: Wikipedia, BLE <https://en.wikipedia.org/wiki/Bluetooth_Low_Energy>

[^96]: Cryptography Engineering Blog, How does Apple (privately) find your offline devices? <https://blog.cryptographyengineering.com/2019/06/05/how-does-apple-privately-find-your-offline-devices/>

[^97]: Wikipedia, Faraday Cage, <https://en.wikipedia.org/wiki/Faraday_cage>

[^98]: Edith Cowan University, A forensic examination of several mobile device Faraday bags & materials to test their effectiveness materials to test their effectiveness <https://ro.ecu.edu.au/cgi/viewcontent.cgi?article=1165&context=adf>

[^99]: Wikipedia, RFID <https://en.wikipedia.org/wiki/Radio-frequency_identification>

[^100]: Wikipedia, NFC <https://en.wikipedia.org/wiki/Near-field_communication>

[^101]: Samsonite Online Shop, RFID accessories, <https://shop.samsonite.com/accessories/rfid-accessories/>

[^102]: Bellingcat, Joseph Mifsud: Rush for the EXIF <https://www.bellingcat.com/news/americas/2018/10/26/joseph-mifsud-rush-exif/>

[^103]: Zoom Support, Adding a watermark <https://support.zoom.us/hc/en-us/articles/209605273-Adding-a-Watermark>

[^104]: Zoom Support, Audio Watermark <https://support.zoom.us/hc/en-us/articles/360021839031-Audio-Watermark>

[^105]: CreativeCloud Extension, IMATAG <https://exchange.adobe.com/creativecloud.details.101789.imatag-invisible-watermark-and-image-monitoring.html>

[^106]: NexGuard, <https://dtv.nagra.com/nexguard-forensic-watermarking>

[^107]: Vobile Solutions, <https://www.vobilegroup.com/solutions>

[^108]: Cinavia, <https://www.cinavia.com/languages/english/pages/technology.html>

[^109]: Imatag, <https://www.imatag.com/>

[^110]: Wikipedia, Steganography <https://en.wikipedia.org/wiki/Steganography>

[^111]: IEEExplore, A JPEG compression resistant steganography scheme for raster graphics images <https://ieeexplore.ieee.org/document/4428921?tp=&arnumber=4428921&url=http:%2F%2Fieeexplore.ieee.org%2Fxpls%2Fabs_all.jsp%3Farnumber%3D4428921>

[^112]: ScienceDirect, Robust audio watermarking using perceptual masking <https://www.sciencedirect.com/science/article/abs/pii/S0165168498000140>

[^113]: IEEExplore, Spread-spectrum watermarking of audio signals <https://ieeexplore.ieee.org/abstract/document/1188746>

[^114]: Google Scholar, source camera identification <https://scholar.google.com/scholar?q=source+camera+identification>

[^115]: HackerFactor Blog, Deanonymizing Tor Circuits <https://www.hackerfactor.com/blog/index.php?/archives/868-Deanonymizing-Tor-Circuits.html>

[^116]: KU Leuven, Website Fingerprinting through Deep Learning <https://distrinet.cs.kuleuven.be/software/tor-wf-dl/>

[^117]: DailyDot, How Tor helped catch the Harvard bomb threat suspect <https://www.dailydot.com/unclick/tor-harvard-bomb-suspect/>

[^118]: ArsTechnica, How the NSA can break trillions of encrypted Web and VPN connections <https://arstechnica.com/information-technology/2015/10/how-the-nsa-can-break-trillions-of-encrypted-web-and-vpn-connections/>

[^119]: ArsTechnica, Does Tor provide more benefit or harm? New paper says it depends <https://arstechnica.com/gadgets/2020/11/does-tor-provide-more-benefit-or-harm-new-paper-says-it-depends/>

[^120]: ResearchGate, The potential harms of the Tor anonymity network cluster disproportionately in free countries <https://www.pnas.org/content/early/2020/11/24/2011893117>

[^121]: arXiv, An Analysis of Anonymity in the Bitcoin System <https://arxiv.org/abs/1107.4524>

[^122]: Bellingcat, How To Track Illegal Funding Campaigns Via Cryptocurrency, <https://www.bellingcat.com/resources/how-tos/2019/03/26/how-to-track-illegal-funding-campaigns-via-cryptocurrency/>

[^123]: Wikipedia, KYC <https://en.wikipedia.org/wiki/Know_your_customer>

[^124]: YouTube, Breaking Monero <https://www.youtube.com/watch?v=WOyC6OB6ezA&list=PLsSYUeVwrHBnAUre2G_LYDsdo-tD0ov-y>

[^125]: Monero, Monero vs Princeton Researchers, <https://monero.org/monero-vs-princeton-researchers/>

[^126]: ArXiv, Tracking Mixed Bitcoins, <https://arxiv.org/abs/2009.14007>

[^127]: Wikipedia, Exploit <https://en.wikipedia.org/wiki/Exploit_(computer_security)>

[^128]: Wikipedia, Freedom Hosting <https://en.wikipedia.org/wiki/Freedom_Hosting>

[^129]: Wired, 2013 FBI Admits It Controlled Tor Servers Behind Mass Malware Attack <https://www.wired.com/2013/09/freedom-hosting-fbi/>

[^130]: Wikipedia, 2020 United States federal government data breach <https://en.wikipedia.org/wiki/2020_United_States_federal_government_data_breach>

[^131]: Wikipedia, Sandbox <https://en.wikipedia.org/wiki/Sandbox_(computer_security)>

[^132]: Wikipedia, CPU <https://en.wikipedia.org/wiki/Central_processing_unit>

[^133]: Wikipedia, Intel Management Engine <https://en.wikipedia.org/wiki/Intel_Management_Engine>

[^134]: Wikipedia, AMD Platform Security Processor <https://en.wikipedia.org/wiki/AMD_Platform_Security_Processor>

[^135]: Wikipedia, IME, Security Vulnerabilities <https://en.wikipedia.org/wiki/Intel_Management_Engine#Security_vulnerabilities>

[^136]: Wikipedia, IME, Assertions that ME is a backdoor <https://en.wikipedia.org/wiki/Intel_Management_Engine#Assertions_that_ME_is_a_backdoor>

[^137]: Wikipedia, IME, Disabling the ME <https://en.wikipedia.org/wiki/Intel_Management_Engine#Disabling_the_ME>

[^138]: Libreboot, <https://libreboot.org/>

[^139]: Digi.Ninja, Jasager <https://digi.ninja/jasager/>

[^140]: Hak5 Shop, Wi-Fi Pineapple <https://shop.hak5.org/products/wifi-pineapple>

[^141]: Wikipedia, Deautentication Attack <https://en.wikipedia.org/wiki/Wi-Fi_deauthentication_attack>

[^142]: Wikipedia, Capture Portal <https://en.wikipedia.org/wiki/Captive_portal>

[^143]: Wired, Why the Security of USB Is Fundamentally Broken <https://www.wired.com/2014/07/usb-security/>

[^144]: Wikipedia, Stuxnet <https://en.wikipedia.org/wiki/Stuxnet>

[^145]: Superuser.com, How do I safely investigate a USB stick found in the parking lot at work? <https://superuser.com/questions/1206321/how-do-i-safely-investigate-a-usb-stick-found-in-the-parking-lot-at-work>

[^146]: Magnet Forensics, Magnet AXIOM <https://www.magnetforensics.com/products/magnet-axiom/cloud/>

[^147]: Cellebrite, Unlock cloud-based evidence to solve the case sooner <https://www.cellebrite.com/en/ufed-cloud/>

[^148]: ArsTechnica, How the way you type can shatter anonymity---even on Tor <https://arstechnica.com/information-technology/2015/07/how-the-way-you-type-can-shatter-anonymity-even-on-tor/>

[^149]: Paul Moore Blog, Behavioral Profiling: The password you can\'t change. <https://paul.reviews/behavioral-profiling-the-password-you-cant-change/>

[^150]: EFF Panopticlick (<https://panopticlick.eff.org/>)

[^151]: ArsTechnica, Stakeout: how the FBI tracked and busted a Chicago Anon <https://arstechnica.com/tech-policy/2012/03/stakeout-how-the-fbi-tracked-and-busted-a-chicago-anon/>

[^152]: Bellingcat MH17 - Russian GRU Commander 'Orion' Identified as Oleg Ivannikov <https://www.bellingcat.com/news/uk-and-europe/2018/05/25/mh17-russian-gru-commander-orion-identified-oleg-ivannikov/>

[^153]: Chromium Documentation, Technical analysis of client identification mechanisms <https://sites.google.com/a/chromium.org/dev/Home/chromium-security/client-identification-mechanisms#TOC-Machine-specific-characteristics>

[^154]: Mozilla Wiki, Fingerprinting <https://wiki.mozilla.org/Fingerprinting>

[^155]: Facebook Research, Deepface <https://research.fb.com/publications/deepface-closing-the-gap-to-human-level-performance-in-face-verification/>

[^156]: Privacy News Online, Putting the "face" in Facebook: how Mark Zuckerberg is building a world without public anonymity <https://www.privateinternetaccess.com/blog/putting-face-facebook-mark-zuckerberg-building-world-without-public-anonymity/>

[^157]: CNBC, "Facebook has mapped populations in 23 countries as it explores satellites to expand internet" <https://www.cnbc.com/2017/09/01/facebook-has-mapped-human-population-building-internet-in-space.html>

[^158]: Bellingcat, Shadow of a Doubt: Crowdsourcing Time Verification of the MH17 Missile Launch Photo <https://www.bellingcat.com/resources/case-studies/2015/08/07/shadow-of-a-doubt/>

[^159]: Brown Institute, Open Source Investigation, <https://brown.columbia.edu/open-source-investigation/>

[^160]: NewScientist, Facebook can recognize you in photos even if you\'re not looking <https://www.newscientist.com/article/dn27761-facebook-can-recognise-you-in-photos-even-if-youre-not-looking/>

[^161]: Google Patent, Techniques for emotion detection and content delivery <https://patents.google.com/patent/US20150242679>

[^162]: TechCrunch, Facial recognition reveals political party in troubling new research <https://techcrunch.com/2021/01/13/facial-recognition-reveals-political-party-in-troubling-new-research/>

[^163]: Slate <https://slate.com/technology/2018/04/facebook-collects-data-on-non-facebook-users-if-they-want-to-delete-it-they-have-to-sign-up.html>

[^164]: The Conversation <https://theconversation.com/shadow-profiles-facebook-knows-about-you-even-if-youre-not-on-facebook-94804>

[^165]: The Verge <https://www.theverge.com/2018/4/11/17225482/facebook-shadow-profiles-zuckerberg-congress-data-privacy>

[^166]: ZDNET <https://www.zdnet.com/article/anger-mounts-after-facebooks-shadow-profiles-leak-in-bug/>

[^167]: CNET <https://www.cnet.com/news/shadow-profiles-facebook-has-information-you-didnt-hand-over/>

[^168]: Anyvision <https://www.anyvision.co/>

[^169]: BBC, Met police deploy live facial recognition technology <https://www.theguardian.com/uk-news/2020/feb/11/met-police-deploy-live-facial-recognition-technology>

[^170]: YouTube, The Economist, China: facial recognition and state control \| The Economist <https://www.youtube.com/watch?v=lH2gMNrUuEY>

[^171]: Washington Post, Huawei tested AI software that could recognize Uighur minorities and alert police, report says <https://www.washingtonpost.com/technology/2020/12/08/huawei-tested-ai-software-that-could-recognize-uighur-minorities-alert-police-report-says/>

[^172]: The Intercept, How a Facial Recognition Mismatch Can Ruin Your Life <https://theintercept.com/2016/10/13/how-a-facial-recognition-mismatch-can-ruin-your-life/>

[^173]: BBC, WhatsApp photo drug dealer caught by \'groundbreaking\' work <https://www.bbc.com/news/uk-wales-43711477>

[^174]: IMDB, Gattaca 1997, <https://www.imdb.com/title/tt0119177/>

[^175]: IMDB, Person of Interest 2011 <https://www.imdb.com/title/tt1839578>

[^176]: IMDB, Minority Report 2002, <https://www.imdb.com/title/tt0181689>

[^177]: Wikipedia, Deepfake <https://en.wikipedia.org/wiki/Deepfake>

[^178]: Wikipedia, Deepfake Events <https://en.wikipedia.org/wiki/Deepfake#Example_events>

[^179]: Joseph Steinberg, How To Prevent Facial Recognition Technology From Identifying You <https://josephsteinberg.com/how-to-prevent-facial-recognition-technology-from-identifying-you/>

[^180]: NIST, Face recognition accuracy with masks using pre-COVID-19 algorithms <https://nvlpubs.nist.gov/nistpubs/ir/2020/NIST.IR.8311.pdf>

[^181]: Wikipedia, Phishing <https://en.wikipedia.org/wiki/Phishing>
...
[^198]: Wikipedia, Faraday Cage, <https://en.wikipedia.org/wiki/Faraday_cage>

[^199]: Edith Cowan University, A forensic examination of several mobile device Faraday bags & materials to test their effectiveness materials to test their effectiveness <https://ro.ecu.edu.au/cgi/viewcontent.cgi?article=1165&context=adf>

[^200]: Privacy International, Timeline of SIM Card Registration Laws <https://privacyinternational.org/long-read/3018/timeline-sim-card-registration-laws>

[^201]: NYTimes, Lost Passwords Lock Millionaires Out of Their Bitcoin Fortunes <https://www.nytimes.com/2021/01/12/technology/bitcoin-passwords-wallets-fortunes.html>

[^202]: Usenix.org, Shedding too much Light on a Microcontroller's Firmware Protection <https://www.usenix.org/system/files/conference/woot17/woot17-paper-obermaier.pdf>

[^203]: Wikipedia, TAILS, <https://en.wikipedia.org/wiki/Tails_(operating_system)>

[^204]: Veracrypt Documentation, Trim Operations <https://www.veracrypt.fr/en/Trim%20Operation.html>

[^205]: Coreboot, <https://www.coreboot.org/>

[^206]: Qubes OS, Anti-Evil Maid, <https://github.com/QubesOS/qubes-antievilmaid>

[^207]: XKCD, Password Strength <https://xkcd.com/936/>

[^208]: QubesOS FAQ, <https://www.qubes-os.org/faq/#is-secure-boot-supported>

[^209]: Wikipedia, Secure Boot, <https://en.wikipedia.org/wiki/Unified_Extensible_Firmware_Interface#Secure_boot>

[^210]: Wikipedia, Booting <https://en.wikipedia.org/wiki/Booting>

[^211]: Wired <https://www.wired.com/2013/12/better-data-security-nail-polish/>

[^212]: Wikipedia, Virtual Machine <https://en.wikipedia.org/wiki/Virtual_machine>

[^213]: Sneak.Berlin , Your computer is not yours <https://sneak.berlin/20201112/your-computer-isnt-yours/>

[^214]: Thenextweb, Apple apps on Big Sur bypass firewalls and VPNs --- this is terrible <https://thenextweb.com/plugged/2020/11/16/apple-apps-on-big-sur-bypass-firewalls-vpns-analysis-macos/>

[^215]: Wikipedia, APNS <https://en.wikipedia.org/wiki/Apple_Push_Notification_service>

[^216]: Wikipedia, Plausible Deniability <https://en.wikipedia.org/wiki/Plausible_deniability>

[^217]: Privacytools.io, Don\'t use Windows 10 - It\'s a privacy nightmare <https://privacytools.io/operating-systems/#win10>

[^218]: Wikipedia, Deniable Encryption <https://en.wikipedia.org/wiki/Deniable_encryption>

[^219]: Wikipedia, Key Disclosure Laws <https://en.wikipedia.org/wiki/Key_disclosure_law>

[^220]: GP Digital, World map of encryption laws and policies <https://www.gp-digital.org/world-map-of-encryption/>

[^221]: Wikipedia, Bitlocker <https://en.wikipedia.org/wiki/BitLocker>

[^222]: Wikipedia, Evil Maid Attack <https://en.wikipedia.org/wiki/Evil_maid_attack>

[^223]: Wikipedia, Cold Boot Attack <https://en.wikipedia.org/wiki/Cold_boot_attack>

[^224]: CITP 2008 (<https://www.youtube.com/watch?v=JDaicPIgn9U>)

[^225]: ResearchGate, Defeating Plausible Deniability of VeraCrypt Hidden Operating Systems <https://www.researchgate.net/publication/318155607_Defeating_Plausible_Deniability_of_VeraCrypt_Hidden_Operating_Systems>

[^226]: SANS.org, Mission Implausible: Defeating Plausible Deniability with Digital Forensics <https://www.sans.org/reading-room/whitepapers/forensics/mission-implausible-defeating-plausible-deniability-digital-forensics-39500>

[^227]: SourceForge, Veracrypt Forum <https://sourceforge.net/p/veracrypt/discussion/technical/thread/53f33faf/>

[^228]: Microsoft, BitLocker Countermeasures <https://docs.microsoft.com/en-us/windows/security/information-protection/bitlocker/bitlocker-countermeasures>

[^229]: SANS, Windows ShellBag Forensics in-depth <https://www.sans.org/reading-room/whitepapers/forensics/windows-shellbag-forensics-in-depth-34545>

[^230]: University of York, Forensic data recovery from the Windows Search Database <https://eprints.whiterose.ac.uk/75046/1/Forensic_Data_Recovery_From_The_Windows_Search_Database_preprint_DIIN328.pdf>

[^231]: A forensic insight into Windows 10 Jump Lists <https://cyberforensicator.com/wp-content/uploads/2017/01/1-s2.0-S1742287616300202-main.2-14.pdf>

[^232]: Wikipedia, Gatekeeper <https://en.wikipedia.org/wiki/Gatekeeper_(macOS)>

[^233]: Wikipedia Veracrypt <https://en.wikipedia.org/wiki/VeraCrypt>

[^234]: OSTIF Veracrypt Audit, 2016, <https://ostif.org/the-veracrypt-audit-results/>

[^235]: Veracrypt Documentation, Unencrypted Data in RAM <https://www.veracrypt.fr/en/Unencrypted%20Data%20in%20RAM.html>

[^236]: Veracrypt Documentation, Data Leaks <https://www.veracrypt.fr/code/VeraCrypt/plain/doc/html/Data%20Leaks.html>

[^237]: Wikipedia, Trim <https://en.wikipedia.org/wiki/Trim_(computing)>

[^238]: Veracrypt Documentation, Trim Operations <https://www.veracrypt.fr/en/Trim%20Operation.html>

[^239]: Veracrypt Documentation, Rescue Disk <https://www.veracrypt.fr/en/VeraCrypt%20Rescue%20Disk.html>

[^240]: St Cloud State University, Forensic Research on Solid State Drives using Trim Analysis <https://repository.stcloudstate.edu/cgi/viewcontent.cgi?article=1141&context=msia_etds>

[^241]: WindowsCentral, Trim Tutorial <https://www.windowscentral.com/how-ensure-trim-enabled-windows-10-speed-ssd-performance>

[^242]: Veracrypt Documentation, Trim Operation <https://veracrypt.eu/en/docs/trim-operation/>

[^243]: Black Hat 2018, Perfectly Deniable Steganographic Disk Encryption <https://i.blackhat.com/eu-18/Thu-Dec-6/eu-18-Schaub-Perfectly-Deniable-Steganographic-Disk-Encryption.pdf>

[^244]: Milan Broz's Blog, TRIM & dm-crypt \... problems? <http://asalor.blogspot.com/2011/08/trim-dm-crypt-problems.html>

[^245]: Veracrypt Documentation, Rescue Disk <https://www.veracrypt.fr/en/VeraCrypt%20Rescue%20Disk.html>

[^246]: Wikipedia, Virtualbox <https://en.wikipedia.org/wiki/VirtualBox>

[^247]: VirtualBox Ticket 17987 <https://www.virtualbox.org/ticket/17987>

[^248]: Whonix Documentation, Spectre Meltdown, <https://www.whonix.org/wiki/Spectre_Meltdown#VirtualBox>

[^249]: Wikipedia, Whonix <https://en.wikipedia.org/wiki/Whonix>

[^250]: Oracle Virtualbox Manual, Snapshots <https://docs.oracle.com/en/virtualization/virtualbox/6.0/user/snapshots.html>

[^251]: Utica College, FORENSIC RECOVERY OF EVIDENCE FROM DELETED ORACLE VIRTUALBOX VIRTUAL MACHINES <https://programs.online.utica.edu/sites/default/files/Neal_6_Gonnella_Forensic_Recovery_of_Evidence_from_Deleted_Oracle_VirtualBox_Virtual_Machine.pdf>

[^252]: Wikipedia, Spectre <https://en.wikipedia.org/wiki/Spectre_(security_vulnerability)>

[^253]: Wikipedia, Meltdown <https://en.wikipedia.org/wiki/Meltdown_(security_vulnerability)>

[^254]: Wikipedia, TOTP <https://en.wikipedia.org/wiki/Time-based_One-time_Password_algorithm>

[^255]: Wikipedia, Multi-Factor Authentication <https://en.wikipedia.org/wiki/Multi-factor_authentication>

[^256]: Qubes OS, System Requirements <https://www.qubes-os.org/doc/system-requirements/>

[^257]: Qubes OS Issues, Simulate Hibernation / Suspend-To-Disk \#2414 <https://github.com/QubesOS/qubes-issues/issues/2414>

[^258]: Wikipedia, TOTP <https://en.wikipedia.org/wiki/Time-based_One-time_Password_algorithm>

[^259]: Wikipedia, Multi-Factor Authentication <https://en.wikipedia.org/wiki/Multi-factor_authentication>

[^260]: Wikipedia, Captcha <https://en.wikipedia.org/wiki/CAPTCHA>

[^261]: Wikipedia, Turing Test <https://en.wikipedia.org/wiki/Turing_test>

[^262]: Google reCaptcha <https://www.google.com/recaptcha/about/>

[^263]: hCaptcha <https://www.hcaptcha.com/>

[^264]: hCaptcha hCaptcha Is Now the Largest Independent CAPTCHA Service, Runs on 15% Of The Internet <https://www.hcaptcha.com/post/hcaptcha-now-the-largest-independent-captcha-service>
...
[^268]: Cloudflare Blog, Cloudflare supports Privacy Pass <https://blog.cloudflare.com/cloudflare-supports-privacy-pass/>

[^269]: Privacy International, Timeline of SIM Card Registration Laws <https://privacyinternational.org/long-read/3018/timeline-sim-card-registration-laws>

[^270]: Wikipedia, Device Fingerprinting <https://en.wikipedia.org/wiki/Device_fingerprint>

[^271]: Developers Google Blog,
...
[^272]: IDMB, The Social Dilemma <https://www.imdb.com/title/tt11464826/>

[^273]: Google Help, Access age-restricted content & features <https://support.google.com/accounts/answer/10071085>

[^274]: Wikipedia, Dark Pattern <https://en.wikipedia.org/wiki/Dark_pattern>

[^275]: The Verge, Tinder will give you a verified blue check mark if you pass its catfishing test <https://www.theverge.com/2020/1/23/21077423/tinder-photo-verification-blue-checkmark-safety-center-launch-noonlight>

[^276]: DigitalInformationWorld, Facebook will now Require you to Create a Video Selfie for Identity Verification! [https://www.digitalinformationworld.com/2020/03/facebook-is-now-demanding-some-users-to-create-a-video-selfie-for-identity-verification.html\#]

[^277]: Vice.com, PornHub Announces \'Biometric Technology\' to Verify Users <https://www.vice.com/en/article/m7a4eq/pornhub-new-verification-policy-biometric-id>

[^278]: Wikipedia, Trust but verify <https://en.wikipedia.org/wiki/Trust,_but_verify>

[^279]: Wikipedia, Faraday Cage, <https://en.wikipedia.org/wiki/Faraday_cage>

[^280]: Edith Cowan University, A forensic examination of several mobile device Faraday bags & materials to test their effectiveness materials to test their effectiveness <https://ro.ecu.edu.au/cgi/viewcontent.cgi?article=1165&context=adf>

[^281]: Wikipedia, Espionage, Organization <https://en.wikipedia.org/wiki/Espionage#Organization>
...
[^318]: DuckDuckGo help, Cache <https://help.duckduckgo.com/duckduckgo-help-pages/features/cache/>

[^319]: DuckDuckGo help, Sources <https://help.duckduckgo.com/duckduckgo-help-pages/results/sources/>

[^320]: Wikipedia, Dead Drop <https://en.wikipedia.org/wiki/Dead_drop>

[^321]: Wikipedia, Secure Communication Obfuscation <https://en.wikipedia.org/wiki/Obfuscation#Secure_communication>

[^322]: Wikipedia, Steganography <https://en.wikipedia.org/wiki/Steganography>

[^323]: Wikipedia, Kleptography <https://en.wikipedia.org/wiki/Kleptography>

[^324]: Wikipedia, Koalang <https://en.wikipedia.org/wiki/Koalang>

[^325]: Tor Documentation, Snowflake, <https://gitlab.torproject.org/legacy/trac/-/wikis/doc/Snowflake/>

[^326]: Privacytools.io, Operating Systems <https://privacytools.io/operating-systems/>

[^327]: Brave Support, What is a Private Window with Tor? <https://support.brave.com/hc/en-us/articles/360018121491-What-is-a-Private-Window-with-Tor->

[^328]: Medium.com, The Windows USN Journal <https://medium.com/velociraptor-ir/the-windows-usn-journal-f0c55c9010e>

[^329]: Medium.com, Digging into the System Resource Usage Monitor (SRUM) <https://medium.com/velociraptor-ir/digging-into-the-system-resource-usage-monitor-srum-afbadb1a375>

[^330]: SANS, Timestamped Registry & NTFS Artifacts from Unallocated Space <https://www.sans.org/blog/timestamped-registry-ntfs-artifacts-from-unallocated-space/>

[^331]: DBAN, <https://dban.org/>

[^332]: NYTimes, Lost Passwords Lock Millionaires Out of Their Bitcoin Fortunes <https://www.nytimes.com/2021/01/12/technology/bitcoin-passwords-wallets-fortunes.html>

[^333]: CrystalDiskInfo <https://crystalmark.info/en/software/crystaldiskinfo/>
  ...
  [Get an anonymous VPN subscription:]: #get-an-anonymous-vpn-subscription
  ...
  [Appendix G: Tor Browser]: #appendix-g-tor-browser
  ...
  [9]: #linux-3
  [10]: #trim-support-1
  [11]: #atanvme-operations-secure-erasesanitize-1
  [12]: #macos-5
  [13]: #trim-support-2
  [14]: #atanvme-operations-secure-erasesanitize-2
  ...
  [15]: media/image1.jpeg
  [www.pornhub.com]: http://www.pornhub.com
  [16]: media/image2.jpeg
  [17]: media/image3.jpeg
  [18]: media/image4.jpeg
  [19]: media/image5.jpeg
  [20]: media/image6.jpeg
  [21]: media/image7.jpeg
  [22]: media/image8.jpeg
  [23]: media/image9.jpeg
  [24]: media/image10.jpeg
  [25]: media/image11.jpeg
  [26]: media/image12.jpeg
  [27]: media/image13.jpeg
  [28]: media/image14.jpeg
  [29]: media/image15.jpeg
  [30]: media/image16.jpeg
  [31]: media/image17.jpeg
  [32]: media/image18.jpeg
  ...
  [33]: media/image19.jpeg
  [34]: media/image20.jpeg
  [35]: media/image21.jpeg
  [36]: media/image22.jpeg
  [37]: media/image23.jpeg
  [38]: media/image24.jpeg
  ...
  [39]: media/image25.jpeg
  [40]: media/image26.jpeg
  [41]: media/image27.jpeg
  [42]: media/image28.jpeg
  [43]: media/image29.jpeg
  [Extra Tools Cleaning:]: #extra-tools-cleaning
  [44]: media/image30.jpeg
  [45]: media/image31.jpeg
  [46]: media/image32.jpeg
