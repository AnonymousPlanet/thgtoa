...
-   [General Preparations:]
    ...
    -   [The Whonix route:]
        ...
        -   [Pick your guest Virtual Machine:]
    -   [The Qubes Route:]
        -   [Installation:]
        -   [Networking setup:]
...
-   [Covering your tracks:]
    -   [Some measures against forensics:]
        -   [TAILS:]
        -   [Windows:]
    -   [How to securely wipe your whole laptop if you want to erase everything:]
        ...
        -   [Windows: ][1]
        -   [MacOS:]
    -   [Removing some traces of your identities on search engine (your digital footprint):]
        -   [Google:][2]
        -   [Bing:]
        -   [DuckDuckGo:]
        -   [Yandex:]
...
-   [Appendix E: Using Gparted to Securely Wipe an SSD drive.]

(Or "How I learned to start worrying and love privacy")

Version 0.3.6 (draft), December 2020 (work in progress, some parts are incomplete) by AnonymousPlanet.
...
Will this guide help you protect your privacy from OSINT researchers like Bellingcat[^7] , Doxing[^8] trolls on 4chan[^9] and others that have no access to the NSA toolbox? More likely. Tho I wouldn't be so sure about 4chan.
...
-   **Be a permanent Adult resident in Germany where the courts have upheld up the legality of not using real names on online platforms (§13 VI of the German Telemedia Act of 2007**[^13]**). Alternatively be an Adult resident of any other country where you can validate and verify the legality of this guide yourself.**
...
-   Using the Tor Anonymity Network[^17] (free).

-   Using an anonymous VPN service[^18] (paid by cash).
...
One could also decide to use a Tor Hidden DNS Service or Oblivious DNS over HTTPS (ODoH)[^32] to further increase privacy/anonymity but unfortunately such services are rare and the only reliable one I know provided by Cloudflare (<https://blog.cloudflare.com/welcome-hidden-resolver/>, https://blog.cloudflare.com/oblivious-dns/). I **[personally]{.ul}** think these are viable technical options but are also a moral choice if you want to use them (despite the risk posed by some researchers[^33]). Here is an illustration showing the current state of DNS privacy:

![][5]

Therefore to mitigate all these issues (as much as possible), this guide we will later recommend a virtualized multi-layered solution of VPN over Tor.

## Your IMEI and IMSI (and by extension, your phone number):

The IMEI (International Mobile Equipment Identity[^34]) and the IMSI (International Mobile Subscriber Identity[^35]) are unique numbers created by mobile phone manufacturers and mobile phone operators.

The IMEI is tied directly to the phone you're using. This number is known and tracked by the mobile phone operators and known by the manufacturers. Every time your phone connects to the mobile network, it will register the IMEI on the network along the IMSI (if a SIM card is inserted but that's not even needed). It's also used by many applications (Banking apps abusing the phone permission on Android for instance[^36]) and smartphone Operating Systems (Android/IOS) for identification of the device[^37]. It is possible but difficult (and not illegal in many jurisdiction[^38]) to change the IMEI on a phone but it's probably easier and cheaper to just find and buy some old (working) Burner phone for a few Euros (this guide is for Germany remember) at a flea market or at some random small shop.
...
-   The smartphone OS makers (Google/Apple for Android/IOs) also keep logs of IMEI/IMSI identifications tied to Google/Apple accounts and which user has been using them. They too can trace back the history of the phone and to which accounts it was tied in the past[^42].

-   Government agencies around the world interested in your phone number can and do use[^43] special devices called "IMSI catchers"[^44] like the Stingray[^45] or the Nyxcell[^46]. These devices are able to impersonate (to spoof) a cell phone Antenna and force a specific IMSI (from your phone) to connect to it to access the cell network. Once they do, they will be able to use various MITM[^47] (Man-In-The-Middle Attacks) that will allow them to:
...
For additional safety (if you absolutely have to take your smartphone along with you or if you want to store your Burner phone), you could consider the use of a faraday cage[^48] bag to store your devices. There are many such faraday "signal blocking" bags available for sale and some of these have been studied[^49] for their effectiveness. If you can't afford such bags, you can probably achieve a "decent result" with one or several sheets of aluminum foil (as shown in the previously linked study).

While there are some smartphones manufacturers like Purism with their Librem series[^50] who have your privacy in mind, they still do not allow IMEI randomization which I believe is a key anti-tracking feature that should be provided.

## Your Wi-Fi MAC address:

The MAC address[^51] is a unique identifier tied to your physical Network Interface (Wired Ethernet or WIFI) and could of course be used to track you if it's not randomized. As it was the case with the IMEI, manufacturers of computers and network cards usually keep logs of their sales (usually including things like: Serial number, IMEI, Mac Addresses, ...) and it's possible again for them to track where and when the computer with the MAC address in question was sold and to whom. Even if you bought it with cash in a supermarket, the supermarket might still have CCTV (or a CCTV just outside that shop) and again the time/date of sale could be used to find out who was there using the Mobile Provider antenna logs at that time (IMEI/IMSI).
...
Operating systems have protections in place to randomize those addresses but are still subject to vulnerabilities[^52].
...
-   IOS/Apple:
    ...
    -   Apple does claim[^53] that they anonymize this data using differential privacy[^54] but you'll have to trust them on that.
...
Geolocation is not only done by using mobile antennas triangulation. It's also done using the WIFIs and Bluetooth devices around you. Operating systems makers like Google (Android[^55]) and Apple (IOS[^56]) maintain a convenient database of most WIFI access points, Bluetooth devices and their location. When your Android smartphone or IPhone is on (and not in Plane mode), it will scan passively (unless you specifically disable this feature in the settings) WIFI access points and Bluetooth devices around you and will be able to geolocate you with more precision than when using a GPS.
...
In addition, recently developed techs could even allow someone to track your movements accurately just based on radio interferences. What this means is that it's possible to track your movement inside a room based on the radio signals passing through. This might seem like a tinfoil hat conspiracy theory claim but here are the references[^57] with demonstrations showing this tech in action: <http://rfpose.csail.mit.edu/> and the video here: <https://www.youtube.com/watch?v=HgDdaMy8KNE>

![][6]

Again such issue could only be mitigated by being in room/building that would act as a faraday cage.

As for the other issues, don't take your smartphone with you where and when you are performing sensitive activities. Just don't. Leave it at home. Or if you absolutely have to take it with you, make sure it's powered off and consider the use of a faraday bag.
...
All of this data is very likely being transmitted, processed and stored indefinitely (unencrypted[^72]) by various third parties[^73].
...
-   Your Fitness Devices and Apps? (Strava[^74]'[^75], Fitbit[^76], Garmin, Polar[^77], ...)[^78]

-   Your Smart Speaker? (Amazon Alexa[^79], Google Echo, Apple Homepod ...).
...
-   IPhones and IPads (IOS 13 and above)[^80]'[^81].

-   Samsung Phones (Android 10 and above)[^82].

-   MacBooks (MacOS 10.15 and above)[^83].

Such devices will continue to broadcast identity information to nearby devices even when off using Bluetooth Low-Energy[^84]. They don't have access to the devices directly (which are not connected to the internet) but instead use BLE to find them through other nearby devices[^85].
...
RFID stands for Radio-frequency identification[^86], it's the technology used for instance for contactless payments and various identification systems. Of course your smartphone is among those devices and has RFID contactless payment capabilities through NFC[^87]. As with everything else, such capabilities can be used for tracking by various actors.
...
The only way to mitigate this problem is to have no RFID tags on you or to shield them again using a type of faraday cage. You could also use specialized wallets/pouches that specifically block RFID communications. Many of those are now made by well-known brands such as Samsonite[^88].

## Your Files Properties/Metadata:

This can be obvious to many but not to all. Most files have metadata attached to them. A good example are pictures which store EXIF[^89] information which can contain a lot of information such as GPS coordinates, which camera/phone model took it and when it was taken precisely. While this information might not directly give out who you are, it could tell exactly where you were at a certain moment which could allow others to use different sources to find you (CCTV or other footage taken at the same place at the same time during a protest for instance). It's important that you verify any file you would put on those platforms for any properties that might contain any information that might lead back to you.

Here is an example of EXIF data that could be on a picture:![][7]
...
Tor and VPNs are not silver bullets. Many advanced techniques have been developed and studied to de-anonymize encrypted traffic over the years[^90]. Most of those techniques are Correlation attacks that will correlate your network traffic in one way or another to logs or datasets. Here are some classic examples:
...
-   Correlation Timing Attacks: As illustrated (simplified) below, an adversary that has access to network connection logs (IP or DNS for instance, remember that most VPN servers and most Tor nodes are known and publicly listed) at the source and at the destination could correlate the timings to de-anonymize you without requiring any access to the Tor or VPN network in between. A real use case of this technique was done by the FBI in 2013 to de-anonymize[^92] a bomb threat hoax at Harvard University. It was also used to de-anonymize
...
Be aware again that this might not be enough against a motivated global adversary[^93] with wide access to global mass surveillance (remember XKEYSCORE, MUSCULAR and PRISM). Such adversary might have access to logs no matter where you are and could use those to de-anonymize you.
...
Lastly, do remember that using Tor in itself can already be considered a suspicious activity[^94] and its use could be considered malicious by some[^95].
...
Contrary to popular belief, Crypto transactions (such as Bitcoin and Ethereum) are not anonymous[^96]. Most crypto currencies can be tracked accurately through various methods[^97].

The main issue is not setting up a random Crypto wallet to receive some currency behind a VPN/Tor address (at this point, the wallet is anonymous). The issue is mainly when you want to convert Fiat money (Euros, Dollars ...) to Crypto and then when you want to cash in your Crypto. You'll have few realistic options but to transfer those to an exchange (such as Coinbase/Kraken/Bitstamp/Binance). Those exchanges have known wallet addresses and will keep detailed logs (due to KYC[^98] financial regulations) and can then trace back those crypto transactions to you using the financial system.

There are some crypto currencies with privacy in mind like Monero but even those can be de-anonymized to some extent[^99][^100].

Even if you use Mixers or Tumblers (services that specialize in anonymizing crypto currencies by "mixing them"), keep in mind this is only obfuscation and not actual anonymity[^101].

## Exploits in your apps and services:

So you're using Tor Browser or Brave Browser with a Tor Tab. You could be using those over a VPN for added security. But you should keep in mind that there are exploits[^102] (hacks) that could be known by an adversary (but unknown to the App/Browser provider). Such exploits could be used to compromise your system and reveal details to de-anonymize you such as your IP address or other details.

A real use case of this technique was the Freedom Hosting[^103] case in 2013 where the FBI inserted malware[^104] using a Firefox browser exploit on a Tor website. This exploit allowed them to reveal details of some users. More recently, there was the notable SolarWinds[^105] hack that breached several US government institutions by inserting malware into an official software update server.
...
To reflect these recommendations, this guide will therefore later guide you in the use of Virtualization so that even if your Browser/Apps get compromised by a skilled adversary, that adversary will find himself stuck in a sandbox[^106] without being able to access identifying information or compromise your system.

## Your CPU:

All modern CPUs[^107] are now integrating hidden management platforms such as the now infamous Intel Management Engine[^108] and the AMD Platform Security Processor[^109].
...
These have already been affected by several security vulnerabilities in the past[^110] that allowed malware to gain control of target systems. These are also accused by many privacy actors including the EFF and Libreboot of being a backdoor into any system[^111].

There are some not so easy ways[^112] to disable the Intel IME on some CPUs and you should do so if you can. For some AMD laptops, you can disable it within the BIOS settings by disabling PSP.

If you're feeling a bit more adventurous, you could install your own BIOS using Libreboot[^113].
...
There are readily available commercial and cheap "badUSB" [^114]devices that can take deploy malware, log your typing, geolocate you, listen to you or gain control of your laptop just by plugging them in. Here are some examples that you can already buy yourself.
...
Such devices can be implanted anywhere (charging cable, mouse, keyboard, USB key ...) by an adversary and can be used to track you or compromise your computer or smartphone. The most notable example of such attacks is probably Stuxnet[^115] in 2005.
...
There are specialized commercial forensics solutions available (Magnet Axiom[^116], Cellebrite Cloud[^117]) that will help an adversary analyze your cloud data with ease.
...
The digital fingerprint is the way you write, the way you behave. The way you click. The way you browse. The fonts you use on your browser[^118]. Fingerprinting is being used to guess who someone is by the way that user is behaving. You might be using specific pedantic words or making specific spelling mistakes that could give you away using a simple Google search for similar features because you typed in a similar way on some Reddit post 5 years ago using a not so anonymous Reddit account.
...
This technology is also widely used in CAPTCHAS[^207] services to verify that you are "human" and can be used to fingerprint a user.
...
A real use and well-documented case of this was the arrest of the hacker Jeremy Hammond[^119] who shared over time several details about his past and was later discovered.

There are also a few cases involving OSINT at Bellingcat[^120].Have a look at their very informative toolkit here: <https://bit.ly/bcattools>

You should never ever share real personal experiences/details that could later lead to you using anonymous identities.

## Your Browser and Device Fingerprints:

Your Browser and Device Fingerprints[^217] are set of properties/capabilities of your System/Browser. These are used on most websites for invisible user tracking but also to adapt the website user experience depending on their browser. For instance websites will be able to provide a "mobile experience" if you're using a mobile browser or propose a specific language/geographic version depending on your fingerprint. Most of those techniques work with recent Browsers like Chromium[^121] based browsers (such as Chrome) or Firefox[^122] unless taking special measures.
...
Even if you're not looking at the camera, they can still figure out who you are[^128] and even make out your emotions[^129].
...
-   Even if you never made a profile on those platforms, you still have one without even knowing it[^130]'[^131]'[^132]'[^133]'[^134].
...
Governments already know who you are because they have your ID/Passport/Driving License pictures and often added biometrics (Fingerprints) in their database. Those same governments are integrating those technologies (often provider by private companies such as the Israeli AnyVision[^135]) in their CCTV networks to look for "persons of interest"[^136]. And some heavily surveilled states like China have implemented widespread use of Facial Recognition for various purposes[^137] including possibly identifying ethnic minorities[^138].
...
We can safely imagine a near future where you won't be able to create accounts or sign-in anywhere without providing unique biometrics (A good time to re-watch Gattaca[^139], Person of Interest[^140] and Minority Report[^141]).

At this time, there are a few steps[^142] you can use to mitigate (and only mitigate) face recognition when conducting sensitive activities where CCTV might be present:

-   Wear a facemask as they have been proven to defeat some face recognition technologies[^143].
...
Phishing[^144] is a type of attack where an adversary could try to extract information from you by pretending to be something/someone else.

A typical case is an adversary using a man-in-the-middle[^47] attack or a falsified e-mail/call to ask your credential for a service. This can be your e-mail or your financial services for example.
...
Forensics techniques are now very advanced and can reveal a staggering amount information from your devices even if they're encrypted[^145]. These techniques are widely used by law enforcement all over the world and should be considered.
...
And last but not least here is this very instructive detailed paper on the current state of IOS/Android security from the John Hopkins University: https://securephones.io/main.html[^146]
...
Any of those providers can be forced to silently (without your knowing (using for example a court order with a gag order[^147] or a national security letter[^148]) log your activity to de-anonymize you. There have been several recent examples of those:

-   2020, The Germany based mail provider Tutanota was forced to implement a backdoor to save unencrypted copies of the e-mails of one user[^149].

-   2017, PureVPN was forced to disclose information of one user to the FBI[^150].

-   2014, EarthVPN user was arrested based on logs provider to the Dutch Police[^151].

-   2014, HideMyAss user was de-anonymized and logs were provider to the FBI[^152].

-   2013, Secure E-Mail provider Lavabit shuts down after fighting a secret gag order[^153].

Some providers have implemented the use of a Warrant Canary[^154] that would allow their users to find out if they have been compromised by such orders but this has not been tested yet as far as I know.

Last but not least, it's now well known that some companies might be sponsored front-ends for some state adversaries (see the Crypto AG story[^155] and Omnisec story[^156]).
...
There are many advanced techniques that can be used by skilled adversaries[^157] to bypass your security measures provided they already know where your devices are. Many of those techniques are detailed here <https://cyber.bgu.ac.il/advanced-cyber/airgap> (Air-Gap Research Page, Cyber-Security Research Center, Ben-Gurion University of the Negev, Israel).
...
This guide will be of little help against such adversaries but there are still some ways to mitigate such techniques:
...
-   If you absolutely want system wide plausible deniability[^166] despite the risks[^158].
...
TAILS[^160] stands for **The Amnesic Incognito Live System.** . It's a bootable Live Operating System running from a USB key that is designed for leaving no traces and forcing all connections through the Tor network.
...
Note: If you can afford it, you might consider getting a Purism Librem laptop: <https://puri.sm>
...
-   Enable BIOS/UEFI password and use a long passphrase[^161] instead of a password if you can.

-   Disable USB/HDMI or any other port (Ethernet, Firewire, SD card ...) if you can.

-   Disable AMD PSP if you can (AMD's equivalent to IME)

-   Check if your laptop BIOS has a secure erase option for your HDD/SDD (convenient in case of need)
...
Here is a good cheap method to make your laptop tamper proof using Nail Polish (with glitter) <https://mullvad.net/en/help/how-tamper-protect-laptop/> [^162] (with pictures).
...
This route will make extensive use of Virtual Machines[^163], they'll require a host OS to run the Virtualization software. You have 3 recommended choices in this part of the guide:
...
This guide will not really recommend using MacOS Big Sur as the host OS due to the "unblockable" telemetry[^164]'[^165] issues. In addition, changes are high that your Mac is or has been tied to an Apple account (at the time or purchase or after signing-in) and therefore its unique hardware identifiers could lead back to you in case of leak.

Linux is not necessarily the best choice for privacy depending on your threat model. This is because using Windows will allow us to use Plausible Deniability[^166] at the OS level.

Windows is a privacy nightmare[^167] but is the only option for using OS wide plausible deniability. Windows telemetry and telemetry blocking is also widely documented which should mitigate many issues.
...
A soft lawful adversary could ask for your encrypted laptop password. At first you could refuse to give out any password (using your "right to remain silent", "right not to incriminate yourself") but some countries are implementing laws[^168] to exempt this from such rights (because terrorists and "think of the children"). In that case you might have to reveal the password or maybe face jail time in contempt of court. This is where plausible deniability will comes into play.
...
Cold Boot attacks[^171] are trickier than the Evil Maid Attack but can be part of an Evil Maid attack as it requires an adversary to come into possession of your laptop while you're actively using your device or shortly afterward.

The idea is rather simple, as shown in this video[^172], an adversary could theoretically quickly boot your device on a special USB key that would copy the content of the RAM (the memory) of the device after you shut it down. If the USB ports are disabled or if they feel like they need more time, they could open it and "cool down" the memory using a spray or other chemicals (liquid nitrogen for instance) preventing the memory decaying. They could then be able to copy its content for analysis. This memory dump could contain the key to decrypt your device. We will later apply a few principles to mitigate these.

In the case of Plausible Deniability. There have been some forensics studies[^173] about technically proving the presence of the hidden data with a simple forensic examination (without a Cold Boot/Evil Maid Attack) but these have been contested by other studies[^174] and by the maintainer of Veracrypt[^175] so I wouldn't worry too much about those yet.
...
-   Windows ShellBags that are stored within the Windows Registry silently storing various histories of accessed volumes/files/folders[^177].

-   Windows Indexing keeping traces of the files present in your User folder by default[^178].

-   Recent lists (aka Jump Lists) in Windows and various apps keeping traces of recently accessed documents[^179]'[^180].
...
-   Gatekeeper[^181] and XProtect keeping track of your download history in a local database
...
Forensics could[^174]'[^177] use all those leaks[^188] to prove the existence of hidden data and defeat your attempts at using plausible deniability and to find out about your various sensitive activities.
...
Forensics cannot extract local data leaks from an OS they can't access. And you will be able to clean most of those traces by wiping the drive or by securely erasing your virtual machines.
...
##### About Hibernation instead of Sleep:

**Hibernation is just better to prevent cold boot attacks. However this feature is notably disabled by default on many Linux distros including Ubuntu. It's possible to enable it but it might not work as expected. Follow this information at your own risk. If you don't want to do this, you should never use the sleep function and power off instead (and probably set the lid closing behavior to power off instead of sleep).**
...
**Note: chances are very high at the moment that this will not work with ARM MacBooks. Due to Virtualization issues.**
...
Again this is to prevent some cold-boot attacks by powering down your RAM and cleaning the encryption key when you close the lid. Once again there are no easy options to do this within the Settings so instead we'll have to do this by running a few commands:
...
You should block OCSP reporting by issuing the following command in Terminal:

-   sudo sh -c \'echo \"127.0.0.1 ocsp.apple.com\" \>\> /etc/hosts\'

(Note: this might not work in Big Sur)
...
This guide will recommend using Tor browser within the host OS because it has the best protections by default. The only other acceptable option in my opinion would be to use Brave Browser (with a Tor tab) but keep in mind that themselves recommend the use of Tor Browser if you feel your safety depends on being anonymous[^183]: " With Tor, Brave works hard to ensure that you're extremely difficult to track online while providing a delightful browsing experience. But if your personal safety depends on remaining anonymous you may wish to use the Tor Browser instead. ".
...
This guide will recommend using Tor browser within the host OS because it has the best protections by default. The only other acceptable option in my opinion would be to use Brave Browser (with a Tor tab) but keep in mind that themselves recommend the use of Tor Browser if you feel your safety depends on being anonymous[^184]: " With Tor, Brave works hard to ensure that you're extremely difficult to track online while providing a delightful browsing experience. But if your personal safety depends on remaining anonymous you may wish to use the Tor Browser instead. ".

This Browser on the host OS will only be used to download various utilities and will never be used for actual sensitive activities.

-   Download and install Tor Browser according to the instruction from <https://www.torproject.org/download/>
...
Veracrypt[^185] is the software we will use for full disk encryption, file encryption and plausible deniability. It is a fork of the well-known but deprecated and unmaintained TrueCrypt. It can be used for
...
It is to my knowledge the only (convenient and usable by anyone) free, open-source and openly audited[^186] encryption software that also provides plausible deniability for general use.
...
-   Encrypt the memory with a Veracrypt option[^187] (settings \> performance/driver options \> encrypt RAM) at a cost of 5-15% performance. This will also disable hibernation (which doesn't clear the key when hibernating) and instead encrypt the memory altogether to mitigate some cold-boot attacks.
...
-   Enable the Veracrypt option to mount volumes as removable volumes (Settings \> Preferences \> Mount volume as removable media). This will prevent Windows from writing some logs about your mounts in the Event logs[^188] and prevent some local data leaks.
...
You should switch your laptop for sleeping to hibernating when closing the lid or when your laptop goes to sleep.
...
-   Route C: Plausible Deniability Encryption of your Operating system (you'll have a "hidden OS" and a "decoy OS" running on the laptop):
    ...
    -   Cons:
        ...
        -   No use with an SSD drive due to requirement of disabling Trim[^189] Operations[^190]. This will severely degrade the performance/health of your SSD drive over time.
...
-   Use a strong passphrase[^161] for the Hidden Volume (obviously a different one than the one for the Outer Volume).
...
-   To rescue disk[^197] or not rescue disk, well that's up to you. I recommend making one (just in case), just make sure to store it outside your encrypted drive (USB key for instance, or wait and see the end of this guide for guidance on safe backups). This rescue disk will not store your passphrase and you'll still need it to use it.
...
This route will use Virtualization and Whonix[^199] as part of the anonymization process. Whonix is a Linux distribution composed of two Virtual Machines:
...
![C:\\Users\\Luc\\AppData\\Local\\Microsoft\\Windows\\INetCache\\Content.Word\\whonix.jpg]
...
Later on you will create and run several Virtual Machines within Virtualbox for your sensitive activities. Virtualbox provides a feature called "Snapshots"[^200] that allow for saving the state of a VM at any point in time. If for any reason later you want to go back to that state, you can restore that snapshot at any moment.
...
You can decide if you prefer to conduct your sensitive activities from a Custom VM that will use the Whonix Gateway or if you will just use the Whonix Workstation provider earlier.
...
-   Whonix Workstation: Coming Soon, I didn't find a suitable tutorial yet.

##### KeePassXC:

You will need something to store your data (logins/passwords, identities and TOTP[^202] information).

For this purpose I strongly recommend KeePassXC because of their TOTP feature. This is the ability to create entries for 2FA[^203] authentication with the authenticator feature.
...
Yes you can actually run MacOS within Virtualbox if you really want to use MacOS. You can run any version of MacOS you want but I wouldn't recommend Big Sur due to their issues with telemetry (as stated earlier) and because it's significantly slower than Catalina on a VM.

Therefore I would personally recommend using MacOS Catalina instead which will just run faster.
...
**Note: I ran in multiple issues with running these on AMD processors. This can be fixed so here is the configuration I used which worked fine with Catalina and Big Sur:**
...
In itself Qubes is the best option in this guide for people who are more comfortable with Linux and tech in general. But it has some downsides such as the lack of OS wide plausible deniability and its hardware requirements and hardware compatibilities. While you can run this on 4GB of RAM as per their requirements[^204], the recommended RAM is 16GB. I would advise against using Qubes OS if you have less than 8GB of RAM.
...
You will need something to store your data (logins/passwords, identities and TOTP[^205] information).

For this purpose I strongly recommend KeePassXC because of their TOTP feature. This is the ability to create entries for 2FA[^206] authentication with the authenticator feature.
...
Captcha[^207] stands for "Completely Automated Public Turing test to tell Computers and Humans Apart" are Turing tests[^208] puzzles you need to complete before accessing a form/website. You'll mostly encounter those provided by Google (reCaptcha service[^209]) and Cloudflare (hCaptcha[^210]). hCaptcha is used on 15% of the internet by their own metrics[^211].
...
While most people think those puzzles are only about solving a little puzzle, it's important to understand that it's much more complex and that modern Captchas uses advanced machine learning and risk analysis algorithms to check if you're human[^212]:

-   They check your browser, cookies and browsing history using Browser fingerprinting[^213].

-   They track your cursor movements (speed, accuracy) and use algorithms to determine if it's "human".

-   They track your behavior before/during/after the tests to ensure you're "human"[^214].
...
Most platforms (including the privacy oriented ones such as Signal/Telegram/ProtonMail will require a phone number to register and most countries now make it mandatory to submit a proof of ID to register[^216].
...
Browser and Device[^217] Fingerprinting are usually integrated into the Captcha services but also in other various services.

Many platforms (like Google[^218]) will check your browser for various capabilities and settings and block Browsers they don't like. This is one of the reasons I recommend using Brave Browser over Tor Browser within your VM.
...
This option is often presented as the "default one", hiding away the "Sign-in with e-mail and password" with clever Dark Patterns[^219] and unfortunately sometimes required.
...
Some platforms/apps will require you to take a live picture of yourself either doing something (a wink, holding an arm up ...) or showing a custom piece of information (a hand written text, a passport or ID) within the picture. Sometimes the platform/app will require several pictures to increase their certainty.![][19]

This guide won't cover this one (yet) as it's mainly used on financial platforms (that will be able to identify you with other means anyway) and some dating apps like Tinder[^220]. Unfortunately this method is now also sometimes being used on Facebook[^221] and Instagram as part of their verification methods (tho I didn't face it yet so far).
...
-   Brave Browser (Chromium based) with a Private Tor Tab has (IMHO) a better acceptance level than Tor Browser (Firefox based). You will have less issues with captchas and online platforms[^218] if you use Brave than if you use Tor Browser (feel free to try this yourself).
...
-   Whonix/Tails: Create/Edit a file /usr/local/etc/torrc.d/50_user.conf[^222].

-   On Tor Browser: Edit the torrc file located at Browser/TorBrowser/Data/Tor[^223].
...
-   Can you create accounts through Tor? Yes but it's very difficult and their onion address[^224] won't help. In most cases you'll just have a random error at sign-up and your account suspended after sign-in.
...
If your account gets suspended [^227] (this can happen on sign-up, after signing-up or after using it in some Google services), you can still get it unsuspended by submitting[^228] an appeal/verification (which will again require your Phone number and possibly an e-mail contact with Google support with the reason). Suspension of the account does not disable the e-mail forwarding but suspended account will be deleted after a while.
...
LinkedIn will then require you go through a verification process that will unfortunately require you to send an ID proof (identity card, passport, driver license). This ID verification is processed by a company called Jumio[^229] that specializes in ID proofing. This is most likely a dead end as this would force you to develop some strong Photoshop skills.
...
They're recommended by privacytools.io[^230]. Their only apparent issue is that they do require (in most cases) a phone number or another e-mail address for registration (when you try to register from a VPN or Tor at least).

They claim they do not store/link the phone/e-mail associated with the registration but only store a hash that is not linked to the account[^231]. As long as their claim is true and the hash is not linked to your account, and that you followed my guide regarding the Burner phone and the pre-paid SIM card, you should be safe from tracking.
...
This time you will need to submit an appeal using a form[^232], provide a good reason and wait for the appeal to be processed by Twitter. During that process, it's possible that you will receive an e-mail (on ProtonMail) asking you to reply to a customer service ticket to prove that you do have access to your e-mail and that it's you. This will be directed toward your Gmail address but will arrive on your ProtonMail.
...
You're going to have to find a different way to post there using proxies that are not known by 4chan blocking system.
...
-   Use an Android Emulator (Youwave[^233], Bluestacks[^234] or if you have a powerful PC, Android Studio) within the Windows VM and run the App through your multi-layer of Tor/VPN. Drawback is that such emulators are usually quite resource hungry and will slow down your VM and use more battery. Here is also an (outdated) guide on this matter: <https://www.bellingcat.com/resources/how-tos/2018/08/23/creating-android-open-source-research-device-pc/>
...
TAILS is great for this, you have nothing to worry about. Shut it down and it's all gone as soon as the memory decays.
...
Windows Event logs will keep many various pieces of information that could contain traces of your activities such as the devices that were mounted (including Veracrypt NTFS volumes for instance[^188]), your network connections, app crash information and various errors. It's always best to clean those up regularly. Do not do this on the Decoy OS.
...
-   The Windows USN journal which stores plenty of information[^235].

-   The Windows System Resource Usage Monitor (SRUM)[^236].

-   Various histories of various programs (such as the recent lists).

-   The free (unallocated) space of your hard drive[^237].
...
-   Download and install PrivaZer from <https://privazer.com/en/download.php>
    ...
    -   Use the Wizard:
        ...
        -   Select Clean Options **(Be careful with this option as it will erase all the free space on the selected partition, especially if you are running the decoy OS. Do not erase the free space or anything else on the second partition as you risk destroying your Hidden OS)**
            ...
            -   If you have an SSD drive, Select SSD with Trim[^189] + 1 pass zero (just to be sure as mentioned earlier, Trim itself should be enough[^192])

            -   If you have an HDD drive, Select HDD with at least 1 pass.

#### Shellbags:

As explained earlier, Shellbags are basically histories of accessed volumes/files on your computer. Remember that shellbags are very good sources of information for forensics[^177] and you need to clean those. Especially if you mounted any "hidden volume" anywhere. Again you shouldn't do this on the Decoy OS.
...
-   Sanitize Overwrite: This method performs a block erase and then overwrite every storage block (it's the same as Block Erase but will overwrite data in addition). This method is probably a bit "overkill".

Obviously for maximum security, Sanitize type options should be preferred. Unfortunately are no **free** easy (bootable with a menu) all-in-one tools available and you'll be left with either going with drive manufacturers provided tools, the free manual hdparm[^239] utility or going with a commercial tool such as PartedMagic[^240] or Blancco[^241].

This guide will therefore recommend the use of the free utility hdparm using a GParted[^242] boot USB.

**Note:** **Before proceeding, you should check your BIOS as some will offer a built-in tool to securely erase your drive. If this is available, you should use that and the following steps won't be necessary. This is for instance the case with some recent Lenovo and Asus laptops. Check this before proceeding to avoid the hassle.**

### Linux:

#### SSD:

See Appendix E: Using Gparted to Securely Wipe an SSD drive.
...
There are several utilities that are recommend (like the old unmaintained DBAN[^243]) for this but personally, I will recommend the use of ShredOS.

Feel free do go with DBAN instead if you want (using this tutorial: <https://www.lifewire.com/how-to-erase-a-hard-drive-using-dban-2619148>), the process is basically the same but might not work out of the box with UEFI laptops.
...
In most cases, if your disk was encrypted with Filevault and you just perform a normal erase, it should be "enough", especially but you should re-install MacOS over it and re-encrypt it with Filevault after re-installing. This should perform a "crypro erase" by overwriting your previous install and encryption.

If you still want to do a separate secure erase (or have no time to perform a re-install and re-encryption). Please use the method described in Appendix E: Using Gparted to Securely Wipe an SSD drive.
...
Contrary to popular belief, it's possible to have some but not all of this information removed by following some simple steps. While this might not remove the information on the websites themselves, it will make it harder for people to find it using search engines.
...
You can check some useful information about how to delete various accounts on this website: <https://backgroundchecks.org/justdeleteme/>

When you're done with this part, you should now handle search engines and while you may not be able to have the information deleted, you can ask them to update/remove outdated information.
...
DuckDuckGo does not store cached version of pages[^244] and will instead forward you to a Google/Bing cached version if available.

In addition, DuckDuckGo source most of their searches from Bing (and not google)[^245] and therefore removing the content from Bing should in time have it removed it from DuckDuckGo too.
...
-   Keep the details of your lawyer nearby or if needed, call him/her in advance to prepare your case.

### If you have no time:

-   Don't Panic.

-   Try to shut down the laptop as soon as possible and hope for the best. If you're fast enough, your memory should decay and your data should be mostly safe for the time being.

-   Contact a lawyer if possible.

# Some OPSEC thoughts:

-   Do not ever use biometrics to safeguard your secrets. Biometrics can be used without your consent.
...
-   Do not plug any equipment in that laptop unless you trust it. Use an USB data blocker.
...
You should also know that most of the digital information described in lengths in this guide can be forged or faked by a motivated adversary for any purpose. Even if you do manage to keep secrets from prying eyes, it is not impossible for anyone to fabricate anything to fit their narrative.
...
-   Digital Evidence can be created, placed, removed or destroyed with ease without leaving traces.

I think it's important that you should not hesitate to question this type of information from any source in this age of disinformation.

Please keep thinking for yourself. Please keep an open mind.
...
As written earlier in this guide and as noted by Privacytools.io[^246], Windows 10 is a privacy nightmare. And disabling everything during and after the installation using the settings available to you is not enough. The amount of telemetry data collected by Microsoft is staggering and could defeat your attempts at keeping secrets. You will need to download and use a couple utilities to (hopefully) force Windows 10 into not sending data back to Microsoft.
...
These instructions are valid for both Linux and Windows.
...
-   Open a terminal from the GParted booted OS (click the Terminal Icon)
...
[^17]: Wikipedia, Tor Anonymity Network <https://en.wikipedia.org/wiki/Tor_(anonymity_network)>

[^18]: Wikipedia, VPN <https://en.wikipedia.org/wiki/Virtual_private_network>
...
[^22]: ArXiv, Characterizing Smart Home IoT Traffic in the Wild <https://arxiv.org/pdf/2001.08288.pdf>

[^23]: Labzilla.io, Your Smart TV is probably ignoring your Pi-Hole <https://labzilla.io/blog/force-dns-pihole>

[^24]: Wikipedia, DNS over HTTPS: <https://en.wikipedia.org/wiki/DNS_over_HTTPS>

[^25]: Wikipedia, DNS over TLS, <https://en.wikipedia.org/wiki/DNS_over_TLS>

[^26]: Wikipedia, Pi-Hole <https://en.wikipedia.org/wiki/Pi-hole>

[^27]: Wikipedia, SNI <https://en.wikipedia.org/wiki/Server_Name_Indication>

[^28]: Wikipedia, eSNI <https://en.wikipedia.org/wiki/Server_Name_Indication#Encrypted_Client_Hello>

[^29]: ZDNET, Russia wants to ban the use of secure protocols such as TLS 1.3, DoH, DoT, ESNI <https://www.zdnet.com/article/russia-wants-to-ban-the-use-of-secure-protocols-such-as-tls-1-3-doh-dot-esni/>

[^30]: ZDNET, China is now blocking all encrypted HTTPS traffic that uses TLS 1.3 and ESNI <https://www.zdnet.com/article/china-is-now-blocking-all-encrypted-https-traffic-using-tls-1-3-and-esni/>

[^31]: KUL, Encrypted DNS=⇒Privacy? A Traffic Analysis Perspective <https://www.esat.kuleuven.be/cosic/publications/article-3153.pdf>

[^32]: ResearhGate, Oblivious DNS: Practical Privacy for DNS Queries <https://www.researchgate.net/publication/332893422_Oblivious_DNS_Practical_Privacy_for_DNS_Queries>

[^33]: Nymity.ch, The Effect of DNS on Tor's Anonymity <https://nymity.ch/tor-dns/>

[^34]: Wikipedia, IMEI <https://en.wikipedia.org/wiki/International_Mobile_Equipment_Identity>

[^35]: Wikipedia, IMSI <https://en.wikipedia.org/wiki/International_mobile_subscriber_identity>

[^36]: Android Documentation, Device Identifiers <https://source.android.com/devices/tech/config/device-identifiers>

[^37]: Google Privacy Policy, Look for IMEI <https://policies.google.com/privacy/embedded?hl=en-US>

[^38]: Wikipedia, IMEI and the Law <https://en.wikipedia.org/wiki/International_Mobile_Equipment_Identity#IMEI_and_the_law>
...
[^41]: Wikipedia, CCTV <https://en.wikipedia.org/wiki/Closed-circuit_television>

[^42]: Apple, Transparency Report, Device Requests <https://www.apple.com/legal/transparency/device-requests.html>

[^43]: The Intercept, How Cops Can Secretly Track Your Phone <https://theintercept.com/2020/07/31/protests-surveillance-stingrays-dirtboxes-phone-tracking/>

[^44]: Wikipedia, IMSI Catcher <https://en.wikipedia.org/wiki/IMSI-catcher>

[^45]: Wikipedia, Stingray <https://en.wikipedia.org/wiki/Stingray_phone_tracker>

[^46]: Gizmodo, Cops Turn to Canadian Phone-Tracking Firm After Infamous \'Stingrays\' Become \'Obsolete\' <https://gizmodo.com/american-cops-turns-to-canadian-phone-tracking-firm-aft-1845442778>

[^47]: Wikipedia, MITM <https://en.wikipedia.org/wiki/Man-in-the-middle_attack>

[^48]: Wikipedia, Faraday Cage, <https://en.wikipedia.org/wiki/Faraday_cage>

[^49]: Edith Cowan University, A forensic examination of several mobile device Faraday bags & materials to test their effectiveness materials to test their effectiveness <https://ro.ecu.edu.au/cgi/viewcontent.cgi?article=1165&context=adf>

[^50]: Purism, Librem 5 <https://shop.puri.sm/shop/librem-5/>

[^51]: Wikipedia, MAC Address <https://en.wikipedia.org/wiki/MAC_address>

[^52]: ResearchGate, Tracking Anonymized Bluetooth Devices <https://www.researchgate.net/publication/334590931_Tracking_Anonymized_Bluetooth_Devices/fulltext/5d3308db92851cd04675a469/Tracking-Anonymized-Bluetooth-Devices.pdf>

[^53]: Apple, Differential Privacy White Paper <https://www.apple.com/privacy/docs/Differential_Privacy_Overview.pdf>

[^54]: Wikipedia, Differential Privacy <https://en.wikipedia.org/wiki/Differential_privacy>

[^55]: Google Android Help, Android Location Services <https://support.google.com/accounts/answer/3467281?hl=en>

[^56]: Apple Support, Location Services and Privacy <https://support.apple.com/en-us/HT207056>

[^57]: State University of New York, Towards 3D Human Pose Construction Using Wi-Fi <https://cse.buffalo.edu/~lusu/papers/MobiCom2020.pdf>
...
[^73]: ZDnet, I asked Apple for all my data. Here\'s what was sent back <https://www.zdnet.com/article/apple-data-collection-stored-request/>

[^74]: Wired, The Strava Heat Map and the End of Secrets <https://www.wired.com/story/strava-heat-map-military-bases-fitness-trackers-privacy/>

[^75]: Bellingcat, How to Use and Interpret Data from Strava\'s Activity Map <https://www.bellingcat.com/resources/how-tos/2018/01/29/strava-interpretation-guide/>

[^76]: The Guardian, Fitness tracking app Strava gives away location of secret US army bases <https://www.theguardian.com/world/2018/jan/28/fitness-tracking-app-gives-away-location-of-secret-us-army-bases>

[^77]: Telegraph, Running app reveals locations of secret service agents in MI6 and GCHQ <https://www.telegraph.co.uk/technology/2018/07/08/running-app-exposes-mi6-gchq-workers-whereabouts/>

[^78]: De Correspondent, Here's how we found the names and addresses of soldiers and secret agents using a simple fitness app <https://decorrespondent.nl/8481/heres-how-we-found-the-names-and-addresses-of-soldiers-and-secret-agents-using-a-simple-fitness-app/412999257-6756ba27>

[^79]: Washington Post, Alexa has been eavesdropping on you this whole time <https://www.washingtonpost.com/technology/2019/05/06/alexa-has-been-eavesdropping-you-this-whole-time/?utm_term=.8514f3a17b1c&itid=lk_interstitial_manual_59>

[^80]: CryptoEngineering, How does Apple (privately) find your offline devices? <https://blog.cryptographyengineering.com/2019/06/05/how-does-apple-privately-find-your-offline-devices/>

[^81]: Apple Support <https://support.apple.com/en-us/HT210515>

[^82]: XDA, Samsung's Find My Mobile app can locate Galaxy devices even when they're offline <https://www.xda-developers.com/samsung-find-my-mobile-app-locate-galaxy-devices-offline/>

[^83]: Apple Support, If your Mac is lost or stolen <https://support.apple.com/en-us/HT204756>

[^84]: Wikipedia, BLE <https://en.wikipedia.org/wiki/Bluetooth_Low_Energy>

[^85]: Cryptography Engineering Blog, How does Apple (privately) find your offline devices? <https://blog.cryptographyengineering.com/2019/06/05/how-does-apple-privately-find-your-offline-devices/>

[^86]: Wikipedia, RFID <https://en.wikipedia.org/wiki/Radio-frequency_identification>

[^87]: Wikipedia, NFC <https://en.wikipedia.org/wiki/Near-field_communication>

[^88]: Samsonite Online Shop, RFID accessories, <https://shop.samsonite.com/accessories/rfid-accessories/>

[^89]: Bellingcat, Joseph Mifsud: Rush for the EXIF <https://www.bellingcat.com/news/americas/2018/10/26/joseph-mifsud-rush-exif/>

[^90]: HackerFactor Blog, Deanonymizing Tor Circuits <https://www.hackerfactor.com/blog/index.php?/archives/868-Deanonymizing-Tor-Circuits.html>

[^91]: KU Leuven, Website Fingerprinting through Deep Learning <https://distrinet.cs.kuleuven.be/software/tor-wf-dl/>

[^92]: DailyDot, How Tor helped catch the Harvard bomb threat suspect <https://www.dailydot.com/unclick/tor-harvard-bomb-suspect/>

[^93]: ArsTechnica, How the NSA can break trillions of encrypted Web and VPN connections <https://arstechnica.com/information-technology/2015/10/how-the-nsa-can-break-trillions-of-encrypted-web-and-vpn-connections/>

[^94]: ArsTechnica, Does Tor provide more benefit or harm? New paper says it depends <https://arstechnica.com/gadgets/2020/11/does-tor-provide-more-benefit-or-harm-new-paper-says-it-depends/>

[^95]: ResearchGate, The potential harms of the Tor anonymity network cluster disproportionately in free countries <https://www.pnas.org/content/early/2020/11/24/2011893117>

[^96]: arXiv, An Analysis of Anonymity in the Bitcoin System <https://arxiv.org/abs/1107.4524>
...
[^98]: Wikipedia, KYC <https://en.wikipedia.org/wiki/Know_your_customer>

[^99]: YouTube, Breaking Monero <https://www.youtube.com/watch?v=WOyC6OB6ezA&list=PLsSYUeVwrHBnAUre2G_LYDsdo-tD0ov-y>

[^100]: Monero, Monero vs Princeton Researchers, <https://monero.org/monero-vs-princeton-researchers/>

[^101]: ArXiv, Tracking Mixed Bitcoins, <https://arxiv.org/abs/2009.14007>

[^102]: Wikipedia, Exploit <https://en.wikipedia.org/wiki/Exploit_(computer_security)>

[^103]: Wikipedia, Freedom Hosting <https://en.wikipedia.org/wiki/Freedom_Hosting>

[^104]: Wired, 2013 FBI Admits It Controlled Tor Servers Behind Mass Malware Attack <https://www.wired.com/2013/09/freedom-hosting-fbi/>

[^105]: Wikipedia, 2020 United States federal government data breach <https://en.wikipedia.org/wiki/2020_United_States_federal_government_data_breach>

[^106]: Wikipedia, Sandbox <https://en.wikipedia.org/wiki/Sandbox_(computer_security)>

[^107]: Wikipedia, CPU <https://en.wikipedia.org/wiki/Central_processing_unit>

[^108]: Wikipedia, Intel Management Engine <https://en.wikipedia.org/wiki/Intel_Management_Engine>

[^109]: Wikipedia, AMD Platform Security Processor <https://en.wikipedia.org/wiki/AMD_Platform_Security_Processor>

[^110]: Wikipedia, IME, Security Vulnerabilities <https://en.wikipedia.org/wiki/Intel_Management_Engine#Security_vulnerabilities>

[^111]: Wikipedia, IME, Assertions that ME is a backdoor <https://en.wikipedia.org/wiki/Intel_Management_Engine#Assertions_that_ME_is_a_backdoor>

[^112]: Wikipedia, IME, Disabling the ME <https://en.wikipedia.org/wiki/Intel_Management_Engine#Disabling_the_ME>

[^113]: Libreboot, <https://libreboot.org/>

[^114]: Wired, Why the Security of USB Is Fundamentally Broken <https://www.wired.com/2014/07/usb-security/>

[^115]: Wikipedia, Stuxnet <https://en.wikipedia.org/wiki/Stuxnet>

[^116]: Magnet Forensics, Magnet AXIOM <https://www.magnetforensics.com/products/magnet-axiom/cloud/>

[^117]: Cellebrite, Unlock cloud-based evidence to solve the case sooner <https://www.cellebrite.com/en/ufed-cloud/>

[^118]: EFF Panopticlick (<https://panopticlick.eff.org/>)

[^119]: ArsTechnica, Stakeout: how the FBI tracked and busted a Chicago Anon <https://arstechnica.com/tech-policy/2012/03/stakeout-how-the-fbi-tracked-and-busted-a-chicago-anon/>
...
[^121]: Chromium Documentation, Technical analysis of client identification mechanisms <https://sites.google.com/a/chromium.org/dev/Home/chromium-security/client-identification-mechanisms#TOC-Machine-specific-characteristics>

[^122]: Mozilla Wiki, Fingerprinting <https://wiki.mozilla.org/Fingerprinting>

[^123]: Facebook Research, Deepface <https://research.fb.com/publications/deepface-closing-the-gap-to-human-level-performance-in-face-verification/>

[^124]: Privacy News Online, Putting the "face" in Facebook: how Mark Zuckerberg is building a world without public anonymity <https://www.privateinternetaccess.com/blog/putting-face-facebook-mark-zuckerberg-building-world-without-public-anonymity/>

[^125]: CNBC, "Facebook has mapped populations in 23 countries as it explores satellites to expand internet" <https://www.cnbc.com/2017/09/01/facebook-has-mapped-human-population-building-internet-in-space.html>
...
[^127]: Brown Institute, Open Source Investigation, <https://brown.columbia.edu/open-source-investigation/>

[^128]: NewScientist, Facebook can recognize you in photos even if you\'re not looking <https://www.newscientist.com/article/dn27761-facebook-can-recognise-you-in-photos-even-if-youre-not-looking/>

[^129]: Google Patent, Techniques for emotion detection and content delivery <https://patents.google.com/patent/US20150242679>

[^130]: Slate <https://slate.com/technology/2018/04/facebook-collects-data-on-non-facebook-users-if-they-want-to-delete-it-they-have-to-sign-up.html>

[^131]: The Conversation <https://theconversation.com/shadow-profiles-facebook-knows-about-you-even-if-youre-not-on-facebook-94804>

[^132]: The Verge <https://www.theverge.com/2018/4/11/17225482/facebook-shadow-profiles-zuckerberg-congress-data-privacy>

[^133]: ZDNET <https://www.zdnet.com/article/anger-mounts-after-facebooks-shadow-profiles-leak-in-bug/>

[^134]: CNET <https://www.cnet.com/news/shadow-profiles-facebook-has-information-you-didnt-hand-over/>

[^135]: Anyvision <https://www.anyvision.co/>

[^136]: BBC, Met police deploy live facial recognition technology <https://www.theguardian.com/uk-news/2020/feb/11/met-police-deploy-live-facial-recognition-technology>

[^137]: YouTube, The Economist, China: facial recognition and state control \| The Economist <https://www.youtube.com/watch?v=lH2gMNrUuEY>

[^138]: Washington Post, Huawei tested AI software that could recognize Uighur minorities and alert police, report says <https://www.washingtonpost.com/technology/2020/12/08/huawei-tested-ai-software-that-could-recognize-uighur-minorities-alert-police-report-says/>

[^139]: IMDB, Gattaca 1997, <https://www.imdb.com/title/tt0119177/>

[^140]: IMDB, Person of Interest 2011 <https://www.imdb.com/title/tt1839578>

[^141]: IMDB, Minority Report 2002, <https://www.imdb.com/title/tt0181689>

[^142]: Joseph Steinberg, How To Prevent Facial Recognition Technology From Identifying You <https://josephsteinberg.com/how-to-prevent-facial-recognition-technology-from-identifying-you/>

[^143]: NIST, Face recognition accuracy with masks using pre-COVID-19 algorithms <https://nvlpubs.nist.gov/nistpubs/ir/2020/NIST.IR.8311.pdf>

[^144]: Wikipedia, Phishing <https://en.wikipedia.org/wiki/Phishing>

[^145]: Grayshirt, <https://www.grayshift.com/>

[^146]: Securephones.io, Data Security on Mobile Devices: Current State of the Art, Open Problems, and Proposed Solutions <https://securephones.io/main.pdf>

[^147]: Wikipedia, Gag Order, <https://en.wikipedia.org/wiki/Gag_order>

[^148]: Wikipedia, National Security Letter <https://en.wikipedia.org/wiki/National_security_letter>

[^149]: Heise Online (German), <https://www.heise.de/news/Gericht-zwingt-Mailprovider-Tutanota-zu-Ueberwachungsfunktion-4972460.html>

[^150]: PCMag, Did PureVPN Cross a Line When It Disclosed User Information? <https://www.pcmag.com/opinions/did-purevpn-cross-a-line-when-it-disclosed-user-information>

[^151]: Internet Archive, Wipeyourdata, "No logs" EarthVPN user arrested after police finds logs <https://archive.is/XNuVw#selection-230.0-230.1>

[^152]: Internet Archive, Invisibler, What Everybody Ought to Know About HideMyAss <https://archive.is/ag9w4#selection-136.0-136.1>

[^153]: Wikipedia, Lavabit Suspension and Gag order, <https://en.wikipedia.org/wiki/Lavabit#Suspension_and_gag_order>

[^154]: Wikipedia, Warrant Canary <https://en.wikipedia.org/wiki/Warrant_canary>

[^155]: Washington Post, The intelligence coup of the century <https://www.washingtonpost.com/graphics/2020/world/national-security/cia-crypto-encryption-machines-espionage/>

[^156]: Swissinfo.ch, Second Swiss firm allegedly sold encrypted spying devices <https://www.swissinfo.ch/eng/second-swiss-firm-allegedly-sold-encrypted-spying-devices/46186432>

[^157]: Wired, Mind the Gap: This Researcher Steals Data With Noise, Light, and Magnets <https://www.wired.com/story/air-gap-researcher-mordechai-guri/>

[^158]: Defuse.ca, TrueCrypt\'s Plausible Deniability is Theoretically Useless <https://defuse.ca/truecrypt-plausible-deniability-useless-by-game-theory.htm>

[^159]: Privacy International, Timeline of SIM Card Registration Laws <https://privacyinternational.org/long-read/3018/timeline-sim-card-registration-laws>

[^160]: Wikipedia, TAILS, <https://en.wikipedia.org/wiki/Tails_(operating_system)>

[^161]: XKCD, Password Strength <https://xkcd.com/936/>

[^162]: Wired <https://www.wired.com/2013/12/better-data-security-nail-polish/>

[^163]: Wikipedia, Virtual Machine <https://en.wikipedia.org/wiki/Virtual_machine>

[^164]: Sneak.Berlin , Your computer is not yours <https://sneak.berlin/20201112/your-computer-isnt-yours/>

[^165]: Thenextweb, Apple apps on Big Sur bypass firewalls and VPNs --- this is terrible <https://thenextweb.com/plugged/2020/11/16/apple-apps-on-big-sur-bypass-firewalls-vpns-analysis-macos/>

[^166]: Wikipedia, Plausible Deniability <https://en.wikipedia.org/wiki/Plausible_deniability>

[^167]: Privacytools.io, Don\'t use Windows 10 - It\'s a privacy nightmare <https://privacytools.io/operating-systems/#win10>

[^168]: Wikipedia, Key Disclosure Laws <https://en.wikipedia.org/wiki/Key_disclosure_law>

[^169]: Wikipedia, Bitlocker <https://en.wikipedia.org/wiki/BitLocker>

[^170]: Wikipedia, Evil Maid Attack <https://en.wikipedia.org/wiki/Evil_maid_attack>

[^171]: Wikipedia, Cold Boot Attack <https://en.wikipedia.org/wiki/Cold_boot_attack>

[^172]: CITP 2008 (<https://www.youtube.com/watch?v=JDaicPIgn9U>)

[^173]: ResearchGate, Defeating Plausible Deniability of VeraCrypt Hidden Operating Systems <https://www.researchgate.net/publication/318155607_Defeating_Plausible_Deniability_of_VeraCrypt_Hidden_Operating_Systems>

[^174]: SANS.org, Mission Implausible: Defeating Plausible Deniability with Digital Forensics <https://www.sans.org/reading-room/whitepapers/forensics/mission-implausible-defeating-plausible-deniability-digital-forensics-39500>

[^175]: SourceForge, Veracrypt Forum <https://sourceforge.net/p/veracrypt/discussion/technical/thread/53f33faf/>

[^176]: Microsoft, BitLocker Countermeasures <https://docs.microsoft.com/en-us/windows/security/information-protection/bitlocker/bitlocker-countermeasures>

[^177]: SANS, Windows ShellBag Forensics in-depth <https://www.sans.org/reading-room/whitepapers/forensics/windows-shellbag-forensics-in-depth-34545>

[^178]: University of York, Forensic data recovery from the Windows Search Database <https://eprints.whiterose.ac.uk/75046/1/Forensic_Data_Recovery_From_The_Windows_Search_Database_preprint_DIIN328.pdf>

[^179]: A forensic insight into Windows 10 Jump Lists <https://cyberforensicator.com/wp-content/uploads/2017/01/1-s2.0-S1742287616300202-main.2-14.pdf>

[^180]: BlackBag, Windows 10 Jump List Forensics <https://www.blackbagtech.com/blog/windows-10-jump-list-forensics/>

[^181]: Wikipedia, Gatekeeper <https://en.wikipedia.org/wiki/Gatekeeper_(macOS)>

[^182]: Brave Support, What is a Private Window with Tor? <https://support.brave.com/hc/en-us/articles/360018121491-What-is-a-Private-Window-with-Tor->

[^183]: Brave Support, What is a Private Window with Tor? <https://support.brave.com/hc/en-us/articles/360018121491-What-is-a-Private-Window-with-Tor->

[^184]: Brave Support, What is a Private Window with Tor? <https://support.brave.com/hc/en-us/articles/360018121491-What-is-a-Private-Window-with-Tor->

[^185]: Wikipedia Veracrypt <https://en.wikipedia.org/wiki/VeraCrypt>

[^186]: OSTIF Veracrypt Audit, 2016, <https://ostif.org/the-veracrypt-audit-results/>

[^187]: Veracrypt Documentation, Unencrypted Data in RAM <https://www.veracrypt.fr/en/Unencrypted%20Data%20in%20RAM.html>

[^188]: Veracrypt Documentation, Data Leaks <https://www.veracrypt.fr/code/VeraCrypt/plain/doc/html/Data%20Leaks.html>

[^189]: Wikipedia, Trim <https://en.wikipedia.org/wiki/Trim_(computing)>

[^190]: Veracrypt Documentation, Trim Operations <https://www.veracrypt.fr/en/Trim%20Operation.html>

[^191]: Veracrypt Documentation, Rescue Disk <https://www.veracrypt.fr/en/VeraCrypt%20Rescue%20Disk.html>

[^192]: St Cloud State University, Forensic Research on Solid State Drives using Trim Analysis <https://repository.stcloudstate.edu/cgi/viewcontent.cgi?article=1141&context=msia_etds>

[^193]: WindowsCentral, Trim Tutorial <https://www.windowscentral.com/how-ensure-trim-enabled-windows-10-speed-ssd-performance>

[^194]: Veracrypt Documentation, Trim Operation <https://veracrypt.eu/en/docs/trim-operation/>

[^195]: Black Hat 2018, Perfectly Deniable Steganographic Disk Encryption <https://i.blackhat.com/eu-18/Thu-Dec-6/eu-18-Schaub-Perfectly-Deniable-Steganographic-Disk-Encryption.pdf>

[^196]: Milan Broz's Blog, TRIM & dm-crypt \... problems? <http://asalor.blogspot.com/2011/08/trim-dm-crypt-problems.html>

[^197]: Veracrypt Documentation, Rescue Disk <https://www.veracrypt.fr/en/VeraCrypt%20Rescue%20Disk.html>

[^198]: Wikipedia, Virtualbox <https://en.wikipedia.org/wiki/VirtualBox>

[^199]: Wikipedia, Whonix <https://en.wikipedia.org/wiki/Whonix>

[^200]: Oracle Virtualbox Manual, Snapshots <https://docs.oracle.com/en/virtualization/virtualbox/6.0/user/snapshots.html>

[^201]: Utica College, FORENSIC RECOVERY OF EVIDENCE FROM DELETED ORACLE VIRTUALBOX VIRTUAL MACHINES <https://programs.online.utica.edu/sites/default/files/Neal_6_Gonnella_Forensic_Recovery_of_Evidence_from_Deleted_Oracle_VirtualBox_Virtual_Machine.pdf>

[^202]: Wikipedia, TOTP <https://en.wikipedia.org/wiki/Time-based_One-time_Password_algorithm>

[^203]: Wikipedia, Multi-Factor Authentication <https://en.wikipedia.org/wiki/Multi-factor_authentication>

[^204]: Qubes OS, System Requirements <https://www.qubes-os.org/doc/system-requirements/>

[^205]: Wikipedia, TOTP <https://en.wikipedia.org/wiki/Time-based_One-time_Password_algorithm>

[^206]: Wikipedia, Multi-Factor Authentication <https://en.wikipedia.org/wiki/Multi-factor_authentication>

[^207]: Wikipedia, Captcha <https://en.wikipedia.org/wiki/CAPTCHA>

[^208]: Wikipedia, Turing Test <https://en.wikipedia.org/wiki/Turing_test>

[^209]: Google reCaptcha <https://www.google.com/recaptcha/about/>

[^210]: hCaptcha <https://www.hcaptcha.com/>

[^211]: hCaptcha hCaptcha Is Now the Largest Independent CAPTCHA Service, Runs on 15% Of The Internet <https://www.hcaptcha.com/post/hcaptcha-now-the-largest-independent-captcha-service>

[^212]: ArsTechnica, "Google's reCAPTCHA turns "invisible," will separate bots from people without challenges" <https://arstechnica.com/gadgets/2017/03/googles-recaptcha-announces-invisible-background-captchas/>

[^213]: BlackHat Asia 2016, "I'm not a human: Breaking the Google reCAPTCHA", <https://www.blackhat.com/docs/asia-16/materials/asia-16-Sivakorn-Im-Not-a-Human-Breaking-the-Google-reCAPTCHA-wp.pdf>

[^214]: Google Blog <https://security.googleblog.com/2014/12/are-you-robot-introducing-no-captcha.html>

[^215]: Cloudflare Blog, Cloudflare supports Privacy Pass <https://blog.cloudflare.com/cloudflare-supports-privacy-pass/>
...
[^217]: Wikipedia, Device Fingerprinting <https://en.wikipedia.org/wiki/Device_fingerprint>

[^218]: Developers Google Blog,
...
[^219]: Wikipedia, Dark Pattern <https://en.wikipedia.org/wiki/Dark_pattern>

[^220]: The Verge, Tinder will give you a verified blue check mark if you pass its catfishing test <https://www.theverge.com/2020/1/23/21077423/tinder-photo-verification-blue-checkmark-safety-center-launch-noonlight>

[^221]: DigitalInformationWorld, Facebook will now Require you to Create a Video Selfie for Identity Verification! [https://www.digitalinformationworld.com/2020/03/facebook-is-now-demanding-some-users-to-create-a-video-selfie-for-identity-verification.html\#]

[^222]: Whonix Documentation, <https://www.whonix.org/wiki/Tor#Edit_Tor_Configuration>

[^223]: Tor Browser Documentation, <https://support.torproject.org/tbb/tbb-editing-torrc/>

[^224]: Facebook Onion Website <http://facebookcorewwwi.onion>

[^225]: Google Help <https://support.google.com/accounts/answer/114129?hl=en>

[^226]: Google Help <https://support.google.com/google-ads/answer/7474263?hl=en>

[^227]: Google, Your account is disabled <https://support.google.com/accounts/answer/40695>

[^228]: Google, Request to restore the account <https://support.google.com/accounts/contact/disabled2>

[^229]: Jumio, ID verification features <https://www.jumio.com/features/>

[^230]: Privacytools.io Recommended E-mail Providers <https://privacytools.io/providers/email/>

[^231]: ProtonMail Human Verification System [https://ProtonMail.com/support/knowledge-base/human-verification/]

[^232]: Twitter Appeal Form <https://help.twitter.com/forms/general>

[^233]: Youwave, <https://youwave.com/>

[^234]: Bluestacks, <https://www.bluestacks.com/>

[^235]: Medium.com, The Windows USN Journal <https://medium.com/velociraptor-ir/the-windows-usn-journal-f0c55c9010e>

[^236]: Medium.com, Digging into the System Resource Usage Monitor (SRUM) <https://medium.com/velociraptor-ir/digging-into-the-system-resource-usage-monitor-srum-afbadb1a375>

[^237]: SANS, Timestamped Registry & NTFS Artifacts from Unallocated Space <https://www.sans.org/blog/timestamped-registry-ntfs-artifacts-from-unallocated-space/>

[^238]: Wikipedia, Wear Leveling <https://en.wikipedia.org/wiki/Wear_leveling>

[^239]: Wikipedia, hdparm <https://en.wikipedia.org/wiki/Hdparm>

[^240]: Wikipedia, Parted Magic <https://en.wikipedia.org/wiki/Parted_Magic>

[^241]: Blancco, Drive Eraser <https://www.blancco.com/products/drive-eraser/>

[^242]: Wikipedia, GParted <https://en.wikipedia.org/wiki/GParted>

[^243]: DBAN, <https://dban.org/>

[^244]: DuckDuckGo help, Cache <https://help.duckduckgo.com/duckduckgo-help-pages/features/cache/>

[^245]: DuckDuckGo help, Sources <https://help.duckduckgo.com/duckduckgo-help-pages/results/sources/>

[^246]: Privacytools.io, Operating Systems <https://privacytools.io/operating-systems/>
  ...
  [Pick your guest Virtual Machine:]: #pick-your-guest-virtual-machine
  ...
  [Some measures against forensics:]: #some-measures-against-forensics
  [TAILS:]: #tails
  [Windows:]: #windows-1
  ...
  [HDD vs SDD:]: #hdd-vs-sdd
  [Linux:]: #linux-1
  [1]: #windows-2
  [MacOS:]: #macos-1
  [Removing some traces of your identities on search engine (your digital footprint):]: #removing-some-traces-of-your-identities-on-search-engine-your-digital-footprint
  [2]: #google-1
  [Bing:]: #bing
  ...
  [Some OPSEC thoughts:]: #some-opsec-thoughts
  ...
  [Appendix E: Using Gparted to Securely Wipe an SSD drive.]: #appendix-e-using-gparted-to-securely-wipe-an-ssd-drive.
  [4]: media/image1.jpeg
  ...
  [8]: media/image5.jpeg
  [9]: media/image6.jpeg
  [10]: media/image7.jpeg
  [11]: media/image8.jpeg
  [12]: media/image9.jpeg
  [hiddenos]: media/image10.jpeg {width="6.5in" height="2.4229166666666666in"}
  [13]: media/image11.jpeg
  [14]: media/image12.jpeg
  [C:\\Users\\Luc\\AppData\\Local\\Microsoft\\Windows\\INetCache\\Content.Word\\whonix.jpg]: media/image13.jpeg {width="7.263888888888889in" height="3.5in"}
  ...
  [17]: media/image16.jpeg
  [18]: media/image17.jpeg
  [19]: media/image18.jpeg
  [20]: media/image19.jpeg
